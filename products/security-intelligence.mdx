...
description: "An outside-in security rating measured from the vendor's own infrastructure — every point deducted traceable to a specific observed configuration."
icon: "shield-halved"
---
...
Four probes read the vendor's live infrastructure on each fact-sheet refresh. All four are passive: they read published records and make ordinary client requests, the same way any browser or mail server would. Nothing is scanned, brute-forced, or authenticated against.
...
<CardGroup cols={2}>
  <Card title="Vendor fact sheet" icon="file-lines">
    The **Security Posture** card carries the score, grade, factor breakdown, and per-factor findings, refreshed on the regular fact-sheet cadence.
  </Card>
...
<Note>
  See also [Vendor Intelligence](/products/vendor-intelligence) for corporate identity validation and the Coverbase Financial Health Score, which are enabled alongside this feature and follow the same beta arrangement.
</Note>
