...
- **Attribution is heuristic, and correcting it is manual.** Disputing an asset fixes one vendor's estate; it does not make the attribution better for the next vendor. Adding an asset the scan cannot discover is the same trade in the other direction.
