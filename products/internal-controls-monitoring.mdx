...
description: "Coverbase against your own applications: evidence collected from live admin consoles, 392 internal controls out of the box, drift detection between runs, and internal findings sharing the same findings, workflow and reporting machinery as third-party risk."
...
## 392 internal controls, ready to run

The [Internal Controls Library](/control-library#internal-controls-library) ships **22 templates spanning 392 controls**, so a program does not start from a blank control set.
...
| Application classes | CRM Application Controls, HRIS & Payroll Application Controls, Financial Services Application Controls |
| Framework-derived | CIS Controls v8.1 SaaS Baseline, citing the CIS safeguard and the NIST SP 800-53 controls behind each control |
...
A material change emails the person who started the inspection, or who owns its schedule. To track it to closure, run **Evaluate** and create a finding from the control, which then has an owner, a due date and the same workflows and dashboards as third-party findings.
...
<Steps>
  ...
  <Step title="Adjudicate and route the exceptions">
    Evaluate produces the judgment against your scale. Create a finding from any control whose evaluation raised an issue, and it gets an owner, a due date and a workflow behind it.
  </Step>
  <Step title="Re-run and watch for drift">
    Subsequent runs compare against the baseline. A material change sends an alert; an immaterial one is noise that does not reach anyone's inbox.
  </Step>
