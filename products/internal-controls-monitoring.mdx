...
<Note>
  Internal control monitoring is delivered by [Coverbase Inspect](/products/agentic-inspect) and is enabled per organization. If you do not see **Inspect** in your navigation, ask your Coverbase team to switch it on. For the access model, hosting and safeguards your security team will ask about, see the [Inspect technical reference](/inspect/overview).
</Note>
...
- **What moved is spelled out.** A changed control lists the records and fields that differ, so a renamed label is distinguishable from a disabled control. Judging a change as material, immaterial or inconclusive is planned; today a reviewer makes that call.
...
A drift result a reviewer elevates becomes a finding like any other: assigned to an owner, tracked to closure, escalated by workflow, and visible in the same dashboards as third-party findings.
...
<Steps>
  ...
  <Step title="Re-run and watch for drift">
    Subsequent runs compare against the baseline and report exactly what changed. A reviewer elevates the changes that matter to findings.
  </Step>
