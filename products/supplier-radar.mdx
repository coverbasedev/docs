...
<CardGroup cols={2}>
  <Card title="Multi-source signal ingestion" icon="rss" href="/products/source-library">
...
<Note>
  Radar configuration (detectors, sources, default reviewers) is managed through the dashboard. Browse the built-in [source library](/products/source-library) and [detector library](/products/detector-library) to see what ships out of the box. Programmatic access to events and detectors is available via the MCP server today; a dedicated REST event stream is on the roadmap.
</Note>