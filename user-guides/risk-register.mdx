...
**History** is the append-only journal: created, proposed, accepted, dismissed, re-scored, owner, status or treatment changed, signal attached, closed or dismissed, merged, edited, scale remapped, closed, reopened. Each row names the person or the agent and shows the scores after the change.
...
| **Radar signals** and **Radar alerts** | At or above **Min severity** (any severity for signals, High for alerts). An alert counts once a reassessment has cited it | Resolved, dismissed or archived |
...
<Frame caption="The Chain tab following Data. Business units on the left, then services, vendors and fourth parties. Red nodes are single points of failure.">
  <img src="/images/user-guides/risk-register-chain.png" alt="Dependency chain graph with columns for business units, services, vendors and fourth parties" />
</Frame>
...
<Warning>
  Editing a scale re-maps existing risks by level **name**; it never re-scores them. A label you move from level 2 to level 3 takes its risks with it, and a name you remove leaves its risks at their number. Each moved risk gets a history row recording the remap.
</Warning>
...
| I renamed a scale level and the scores changed. | They did not; the risks followed their level **name** to its new number. Check the remap rows in each risk's history. |
...
<CardGroup cols={2}>
  <Card title="Risk geography map" icon="map-location-dot" href="/user-guides/risk-geography-map">
    The Geography tab: supplier sites, country counts and the vendor filters.
  </Card>
