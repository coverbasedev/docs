...
A second refresh on the same day keeps comparing against the earlier day rather than against the morning's scan, so running a refresh by hand after the scheduled one does not wipe the panel. Each count is the full number that moved.
