...
<Note>
  Coverbase also connects to [DocuSign](/integrations/guides/docusign) if your organization already signs there. The two are alternatives, not layers: DocuSign creates the envelope in your own DocuSign account, while everything on this page happens inside Coverbase. Pick one per agreement rather than both. On a contract record both start from the **Signature** step of the **Workflow** tab, once the contract has cleared its approval chain; see [Contract intake and approval](/user-guides/contract-intake-and-approval).
</Note>
