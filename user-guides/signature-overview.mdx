...
Coverbase signs agreements in the same place it stores them. You do not export a PDF, send it somewhere else, and file the executed copy back by hand. The contract, the signature, the evidence that it happened, and the sealed document all live on the same record.
...
Each recipient carries their own state alongside it — *not yet notified*, *notified*, *opened*, *consented*, *signed*, *completed*, *declined*, *delegated* — which is what the envelope's **Signing Order** panel reads out party by party.
...
Signing settings are organization-wide and only need doing once. They are the difference between an agreement that reads as yours and one that reads as generic software.
...
Every envelope carries three tabs, and the last two are the ones that matter months later.
...
<Tip>
  The audit trail is the answer to "prove this person signed this document on this date." It records the source document's SHA-256 before anyone touched it and the executed document's SHA-256 after sealing, so a reader can check the file they are holding against the record.
</Tip>
