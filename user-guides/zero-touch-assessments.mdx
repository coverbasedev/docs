...
<Steps>
  ...
  <Step title="Work the controls">
    The **Controls** tab sits first among the five deep dives, beside Registrations, Security, People and Financial — the control evaluation is one of the things the composite is made of, not a footnote to it. See [Working the controls](#working-the-controls) below.
  </Step>
...
<Warning>
  **Not enough information is not a finding against the vendor.** It says the question cannot be answered from outside, which is exactly what a full assessment is for. Treating it as a failure is the single most common way to misread one of these runs — and it is why it has its own view rather than being folded in with what was found.
</Warning>
...
<AccordionGroup>
  ...
  <Accordion title="Why does a control show a code like ZT-SEC-04?">
    That is the control's identifier, and it is stable — quote it in a ticket or match it against an export. Codes are numbered within their section, so the letters tell you the area at a glance.
  </Accordion>
  ...
  <Accordion title="The profile describes the wrong company. Can I fix it?">
    Yes. Correct the vendor from **Actions → Reassess** — tell it the right name or website in your own words — then use **Re-source profile** on the result page. Re-scoring alone deliberately keeps the existing profile, so correcting identity is the step that refreshes it.
  </Accordion>
