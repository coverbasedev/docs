...
### Three states we cannot reach at all
...
Go to **Configuration → Radar**, open the **Sources** tab, and click **Add source → From Source Library**. Choose the **Breach Notification & Disclosure** segment.

You can add all seventeen at once with **Add all visible**, or start with a few. You can also skip this step: enabling the detector in Step 2 adds every registry you have not added yet. Ways to choose:
...
Open the **Detectors** tab and click **Add detector → From detector library**. Search for **Breach Notification Filings** (it sits in the **Cybersecurity** segment) and click **Configure** to add it and open it for editing, or **Add detector** to add it with its defaults.

Enabling it adds the seventeen breach registries, plus **HIPAA Journal Breach News**, to your sources where you do not have them already, and scopes the detector to them. The library preview lists them under **Sources**.
...
In the editor, check two settings:

- **Sources.** The detector arrives scoped to the registries it deployed with. Remove any you do not want. A source you add later is not read by this detector until you add it here.
...
Give it a pull cycle. Registries are checked every six hours, and the first pull of a large register such as Oregon brings in its full history, so expect a burst on day one and a much smaller trickle after that.

Go to **Radar → Signals**. A signal from a registry looks different from a news signal: the summary is the filing's own fields rather than prose. That is expected. Triaging what arrives works the same way as any other signal, which the [Radar signals guide](/user-guides/radar-signals) covers.
...
| --- | --- |
| Sources are added but nothing alerts. | No detector reads them. Enable **Breach Notification Filings** (Step 2), or add the sources to an existing detector's **Sources**. |
| A burst of old filings on the first day. | The first pull of a large register such as Oregon brings in its history. Later pulls only add new rows. |
| A registry shows **Unavailable** in the source library, or **Paused** in your sources. | Coverbase withdrew it because the page no longer exists, refuses automated requests, or could not be read. It stays listed with the reason. |
| The same vendor appears several times in a week. | One breach filed in several states. Triage it once. |
| Massachusetts or New Hampshire results look like an error page. | Both block automated requests and were not confirmed against the live register. Tell Coverbase support and rely on the other registries meanwhile. |
