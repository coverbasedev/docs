...
The **Breach Notification & Disclosure** group in the source library holds twelve
...
Iowa, Wisconsin, Montana, Massachusetts, New Hampshire, and the HHS OCR
healthcare portal all publish something, but none of them can currently be read
automatically. Some have moved or removed the page, some load their register
only after you click through a portal, and some block automated access outright.
They are not in the library because a source that silently collects nothing is
worse than an absent one. If any of these matter to you, tell us and we will
prioritize the work.
...
You can add all twelve at once, or start with a few. Three ways to choose:

- **Add California, Vermont, Oregon, and Maryland first.** These are the largest registers, so they give you the most coverage for the least setup.
