...
Approvals stall on absence more often than on disagreement. Coverbase routes work to a **user group** rather than to a person wherever you want it to survive someone's holiday, and records both the group that was asked and the individual who ended up accountable.
...
<Steps>
  ...
  <Step title="They return without an admin touching anything">
    The window expires on its own. Nobody has to remember to re-enable a colleague on the Monday they get back, which is where manual delegation schemes usually leak.
  </Step>
