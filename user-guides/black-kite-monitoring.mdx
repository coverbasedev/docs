...
- Admin access to **Configuration → External Integrations**, with permission to update integrations.
...
Go to **Configuration → External Integrations** and choose **Black Kite**.

Paste your client ID and client secret and click **Save Black Kite Configuration**. Then click **Test connection**, which makes one harmless call to Black Kite and confirms the credentials work. Do it now rather than finding out from a week of failed overnight runs.
...
Once credentials are saved, the **Radar monitoring** section below them unlocks. Until then it reads *Save your Black Kite credentials above to configure monitoring.*
...
A nightly run skips the portfolio companies it cannot attribute to any vendor, and the app does not list them. They are usually companies you monitor in Black Kite but have not created as vendors in Coverbase, or vendors whose recorded website never lined up. To check a vendor you expected to see, open it and use **Check Black Kite now** on its Black Kite card.
...
You can also start from the detector library under **Configuration → Radar → Detectors → Add detector → From detector library**. Search for *Black Kite* and you will find five ready-made policies, each marked with a Black Kite badge. They are already wired to your source, and cannot be added until the source exists.
...
**Delete Configuration** removes the credentials and stops everything: no further runs, and no further alerts. Existing alerts and their history are kept.

To pause alerting while keeping collection, disable the detector instead. To stop collecting while keeping your settings, archive the Black Kite source under **Configuration → Radar → Sources**.
