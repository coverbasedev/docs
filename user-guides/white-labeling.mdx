...
Once the domain is live, the address shows as **Outbound sender email** under **Organization identity** on **Configuration → Communications**. Only Coverbase can change that field, so ask your Coverbase contact when the address needs to move. Admins can edit the sender name and the inbound contact email on the same page themselves.
