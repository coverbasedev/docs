...
<Note>
  This is a gated feature. If you do not see a **Know-Your-Vendor** tab on your vendors, it has not been switched on for your organization yet. Ask your Coverbase contact.
</Note>
...
<CardGroup cols={3}>
  <Card title="On the vendor" icon="building">
    A **Know-Your-Vendor** tab on every vendor. **Onboarding** holds the questionnaire sent to the vendor and its verification checks; **Records on file** holds bank accounts, sites, tax registrations, diversity records, legal entity assignments, pending changes, and the full change history.
  </Card>
  <Card title="In a questionnaire" icon="clipboard-list">
    Four question types you can drop into any Know-Your-Vendor questionnaire. This is how suppliers fill things in.
  </Card>
  <Card title="In configuration" icon="sliders">
    **Configuration → Know Your Vendor Settings** for the policy, and **Configuration → Legal entities** for the entities suppliers get assigned to.
  </Card>
</CardGroup>

<Frame caption="Records on file, on a vendor's Know-Your-Vendor tab. Bank accounts are masked to the last four digits until someone with the reveal permission asks to see one.">
  <img src="/images/user-guides/supplier-know-your-supplier-tab.png" alt="The Records on file section of a vendor's Know-Your-Vendor tab, showing masked bank accounts with validation badges, sites, tax registrations and the change history." />
</Frame>
...
When you author a questionnaire on the **Know-Your-Vendor** tab of **Configuration → Questionnaires**, four extra response types appear alongside the usual text and select types:

| Question type | What the supplier fills in |
| --- | --- |
| **Bank Account** | Country, then that country's bank fields, plus IBAN and SWIFT where they apply |
| **Site / Address** | An address, with the postal code checked against the national format, and the email addresses purchase orders and remittance advice go to |
...
| Payment method | Whether the account is cross border. An account held in a country none of the supplier's assigned legal entities pay from goes out by wire; an account in one of their countries is domestic. A supplier with no legal entity assignment gets no payment method until one is assigned. |
...
A supplier who is exempt from a registration has no number to give. Ticking the box that says the supplier is exempt from that registration replaces the identifier with a **Reason for exemption**, which is required. The two are alternatives: a submission carrying both is refused.
...
The queue is the **Vendor changes** page. Open it from the **Vendor changes** button at the top of the vendor page, which shows a count when something is waiting. The same page also lists subprocessor suggestions waiting for a decision.

<Frame caption="The bank changes on the Vendor changes page. Each pending change shows what is being changed, who submitted it, and through which surface.">
  <img src="/images/user-guides/supplier-changes-queue.png" alt="The Vendor changes page listing pending bank account changes awaiting review, with approve and reject actions." />
</Frame>
...
The **Know Your Vendor** section of **Configuration → Know Your Vendor Settings** holds the policy. Everything on it is per organization.

<Frame caption="Know Your Vendor settings. Bank review is on by default.">
  <img src="/images/user-guides/supplier-kys-settings.png" alt="The Know Your Vendor section of the configuration page, showing review and approval settings, collection options, limits and update request defaults." />
</Frame>
...
The 109 countries ship with rules already configured, and those are documented per country in [Supplier countries](/user-guides/supplier-countries/overview). Coverbase can override any country's rules for your organization's suppliers, which is how a late correction from your finance team becomes a configuration change rather than a release. Ask your Coverbase contact.
...
| **Registry check pending** or **Registry unavailable** on a tax registration | The background check runs every 15 minutes. **Registry unavailable** means the register could not be reached; it retries on its own, and **Retry verification** runs it again by hand. It never blocks anything. |
...
| "BRL is held for suppliers in BR only" | BRL is held for suppliers in Brazil. The test is the supplier's own country, not the bank's. |
