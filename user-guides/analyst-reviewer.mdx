...
1. Open **Radar → Signals**. The **Needs attention** view lists every signal in alert, most recently alerted first. Each signal shows the highest severity across its alerts, the vendors involved, and the detectors that fired. One event is one signal, however many detectors hit it.
2. Open a signal. The status card says where it stands, and the **Vendors** card lists which of your vendors are linked to it. The summary says what happened, when, and when you were alerted. The **Detectors** tab lists each alert with the detector's reasoning and confidence, **Sources** holds the articles and advisories behind it, and **Timeline** shows the order things happened.
3. Act on it from the **Actions** menu. **Open case** for anything that needs investigation or vendor outreach (a case is preliminary, not yet a confirmed problem). **New finding** when it is confirmed, with an owner and a due date. **Start reassessment** when the vendor's risk picture has changed enough to re-run the plan.
...
5. Dismiss what does not matter. Dismissing clears every alert on the signal. Tick **Also dismiss future similar alerts** and the detector learns from it, so the next one like it stays quiet.
6. If a detector keeps firing on non-actionable items, say so. Detector guidance and severity thresholds are quick admin fixes.

A **Tracked** signal has no alert behind it. A vendor you depend on is linked to the event, so Radar shows the exposure and waits. It becomes work only when a detector fires or you act on it.

<Frame caption="The Signals tab. One signal per event, with the highest severity across its alerts, the vendors involved, and the detectors that fired.">
...
<Frame caption="A signal. Status and linked vendors on the left. On the right, the generated summary, when the event happened against when you were alerted, and the detectors that fired.">
  <img src="/images/user-guides/analyst-radar-signal-detail.png" alt="Radar signal page showing the status card, generated summary, and detector alerts" />
</Frame>
...
<AccordionGroup>
  ...
  <Accordion title="Where do Coverbase Library documents come from, and do ours go into it?">
    Coverbase collects them itself: our agent requests documentation directly from the vendor and retrieves what the vendor publishes on its trust center, and a Coverbase reviewer approves each one before it appears. We don't scrape arbitrary web sources, because that material tends to be outdated and low quality. Nothing your organization uploads, and nothing a vendor sends you through the portal, is ever added to the library. See [Document library](/products/document-library).
  </Accordion>
