...
<Info>
  This guide is part of the [User Guides](/user-guides/overview) collection. It is for analysts, reviewers, and InfoSec SMEs doing daily vendor reviews. It assumes your environment is already configured. If you are setting it up, start with the [Admin and setup guide](/user-guides/admin-setup).
...
Coverbase reads vendor documents and measures them against your controls, so your job shifts from hunting through PDFs to judging results. A typical assessment measures a hundred-plus controls and raises a handful of material issues. In one real case, 111 controls produced 5.

This is the loop you run:
...
The left panel tracks the assessment through four stages (**Collecting Documents**, **Analyzing Controls**, **Review Results**, **Quality Control**) and always tells you what it is waiting for, with a shortcut into that work.
...
**Risk Scoring Changes** expands to show how this assessment moved the vendor's scores. Check it before you confirm. On a service-scoped assessment the change applies to the services. It reaches the vendor only if your organization rolls service scores up.
...
1. Open **Radar → Signals**. The **Needs attention** view lists every signal in alert, most recently alerted first. Each signal shows the highest severity any detector assigned, the vendors involved, and the detectors that fired. One event is one signal, however many detectors hit it.
2. Open a signal. The status card says where it stands, and the **Vendors** card lists which of your vendors are linked to it. The summary says what happened, when, and when you were alerted. The **Detectors** tab lists each detector that fired, with its reasoning and confidence, **Sources** holds the articles and advisories behind it, and **Timeline** shows the order things happened.
...
<Frame caption="The Signals tab. One signal per event, with the highest severity any detector assigned, the vendors involved, and the detectors that fired.">
  <img src="/images/user-guides/analyst-radar-signals.png" alt="Radar Signals tab listing signals with status, severity, vendors, and detectors" />
</Frame>

<Frame caption="A signal. On the left, its status and the vendors exposed. On the right, the generated summary, the time from event to alert, and the detectors that put the signal in alert.">
  <img src="/images/user-guides/analyst-radar-signal-detail.png" alt="Radar signal page showing the status card, generated summary, and the detectors that fired" />
</Frame>
...
<Frame caption="Radar cases. Preliminary investigations tagged to affected vendors, ready to close, promote to a finding, or trigger a reassessment.">
  <img src="/images/user-guides/analyst-radar-cases.png" alt="Radar cases tab" />
</Frame>
...
As a reviewer, you validate extractions against source language, assign owners and due dates, and send attestation requests to business stakeholders through a trimmed-down portal.
...
<Note>
  The full obligations workflow (validation, ownership, and stakeholder attestations) gets its own dedicated guide, which we will add to this collection.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="I assessed a service and the vendor's score didn't move. Why?">
    The score landed on the service. An assessment writes its residual risk to whatever it was scoped to, so a service-scoped assessment leaves the vendor's score alone unless your organization has **Risk Roll-up** turned on. With it on, the vendor takes the highest score among its live services. See [Where a vendor's score comes from](/user-guides/running-an-assessment#where-a-vendors-score-comes-from).
  </Accordion>
  ...
  <Accordion title="Do vendors respond to the portal?">
  ...
  <Accordion title="Where do Coverbase Library documents come from, and do ours go into it?">
    Coverbase collects them itself: our agent requests documentation directly from the vendor and retrieves what the vendor publishes on its trust center, and a Coverbase reviewer approves each one before it appears. We do not scrape arbitrary web sources, because that material tends to be outdated and low quality. Nothing your organization uploads, and nothing a vendor sends you through the portal, is ever added to the library. See [Document library](/products/document-library).
  </Accordion>
