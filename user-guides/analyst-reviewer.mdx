...
**Risk Scoring Changes** expands to show how this assessment moved the vendor's scores. Worth a look before you confirm. On a service-scoped assessment the change lands on the services, and reaches the vendor only if your organization rolls service scores up.
...
<AccordionGroup>
  ...
  <Accordion title="I assessed a service and the vendor's score didn't move. Why?">
    Because the score landed on the service. An assessment writes its residual risk to whatever it was scoped to, and a service-scoped assessment leaves the vendor's own score alone unless your organization has turned on **Risk Roll-up**. With it on, the vendor takes the highest score among its live services. See [Where a vendor's score comes from](/user-guides/running-an-assessment#where-a-vendors-score-comes-from).
  </Accordion>
