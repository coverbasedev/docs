...
1. Open **Radar → Signals**. The **Needs attention** view lists every signal in alert, most recently alerted first. Each row carries the highest severity across its alerts, the vendors involved, and the detectors that fired. One event is one row, however many detectors hit it.
...
<Frame caption="The Signals tab. One row per event, with the highest severity across its alerts, the vendors involved, and the detectors that fired.">
