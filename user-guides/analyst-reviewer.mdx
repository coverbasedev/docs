...
2. Open a signal. The status card says where it stands. The summary says what happened and which of your vendors it touches. The **Detectors** tab lists each alert with the detector's reasoning, and **Exposure** shows how you are connected when the vendor named in the event is not one of yours.
3. Act on it. **Open case** for anything that needs investigation or vendor outreach (a case is preliminary, not yet a confirmed problem). **New finding** when it is confirmed, with an owner and a due date. **Start reassessment** when the vendor's risk picture has changed enough to re-run the plan.
...
<Frame caption="A signal. Status and summary on the left; the detectors that fired, your exposure path, sources, timeline, notes, and activity across the tabs.">
