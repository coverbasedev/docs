...
**Radar** in the left navigation opens the **Portfolio risk** chart (once there are signals to plot) and three tabs: **Signals**, **Cases** and **Reassessments**. At the right of the tab bar, the **Configurations** menu links to the three tabs of **Configuration → Radar**: **Detectors**, **Sources** and **Reassessments** (the reassessment triggers). Beside it, **New detector** offers **From detector library** and **Create custom detector**, and under **Also add**, **Source from library** and **Log a signal manually**.
...
Three built-in views sit in the view selector. **All Signals** is the default and shows everything, newest first. **In Alert** shows only signals with an active alert, most recently alerted first. This is the queue. **Triaged** shows signals someone has already acted on. Search signal titles and summaries, add filters, change sorting and columns under **Display**, and save the arrangement with **New view**.
...
| **Nth Parties**, **Nth Party Tags** | The companies your vendors depend on that the event names, and their tags. Hidden by default; turn them on under **Display**. |

Rows group by **Today**, **Yesterday**, **This week**, **Last week** and **Older**. The icon button beside **Display** collapses or expands every group. The **Actions** menu offers **Download as Excel**: the **Export Signals** dialog exports every signal in the view with the table's visible columns, and lets you also include the summary, notes and detector results.

To record an event by hand, choose **New detector → Log a signal manually**. Give it a **Title** and **Summary**, and optionally the **Impact area**, **When it occurred**, **Risk domains**, a **Source URL** and **Source name**, and the **Affected vendors**. **Create signal** opens the new signal, and the detectors evaluate it like any other event.
...
<Frame caption="The dismiss dialog. Every active alert is ticked by default; the signal is dismissed only when all of them are.">
  <img src="/images/user-guides/radar-signals-dismiss-dialog.png" alt="Dismiss signal dialog with a reason field, the alerted detectors, and the signal's alerts all selected" />
</Frame>

The dialog names the **Alerted Detectors** with how many alerts each raised, then lists the signal with a count such as **22 of 22 alerts**. Every active alert is ticked. Expand the signal (or **Expand all**) and untick the alerts you want to keep, and the button changes from **Dismiss signal** to **Dismiss N alerts**; the signal itself is dismissed only when every alert is. Type a **Reason (optional)** if you want one on the record. Mention a colleague in it and they get a **Dismissal mentions** notification.

Each dismissed alert also becomes a negative example on the detector that raised it, with your reason attached, so events like it stay quiet next time. That happens only when you can edit detectors; a dismissal by someone who cannot closes the alerts and teaches nothing. **Restore (un-dismiss)** takes the example back out. Dismissing is the main way a detector learns; the others are the thumbs on each alert and the detector's own **Test detector** run, below.

### Opening a case

**Create a Case** asks for a **Title** (prefilled with the signal title), a **Description** (prefilled with its summary), and the **Affected Vendors**. **Include all affected vendors** is on by default. **Create and Continue** takes you to **Send Emails**, where a **Shared Draft Template** with placeholders for the vendor name and your organization name can be applied to every pending vendor, edited per vendor, and sent one at a time or with **Send all remaining**. **Skip and Go to Case** leaves the emails for later.

The case page has **Overview**, **Vendors**, **Notes** and **Activity** tabs, with **Send Emails**, **Add Vendors** and **Finalize Case** at the top and **Export as Excel**, **Import Vendors** and **Archive Case** in the menu beside them. Each vendor thread carries a status, an assignee, a due date and the vendor's replies. **Finalize Case** closes the case as resolved. While vendor threads are still open it asks first: **Resolve All and Close**, or **Close Without Resolving** to leave them as they are. **Reopen Case** undoes a close. **Archive Case** is final.

### Starting a reassessment

**Start a reassessment** lists the exposed vendors. Remove any you do not want, **Add vendor...** for one Radar missed, optionally pick an **Assessment Plan**, and click **Start a reassessment**. The draft opens on its own page and is listed under **Radar → Reassessments**. Check the vendors and the plan there, then **Run reassessment**, which creates one assessment per vendor from the plan. **Dismiss** closes the draft without running it. Reassessments can also be raised without a person, by the triggers under **Configuration → Radar → Reassessments**: each trigger names a plan, a vendor scope, and either risk-score conditions or an **AI event-type instruction** describing the kind of signal that should trigger it.

### The risk review

When a firing detector has **Residual risk impact** switched on, acknowledging, creating a finding, starting a reassessment, or finalizing a case raises the affected vendors' residual risk. **Review vendor risk profile updates** opens with the new scores, overall and per domain. Adjust any of them and **Save and continue**, or **Continue** to accept. Dismissing never moves risk. How the size of the move is chosen is on the [Supplier Radar page](/products/supplier-radar#risk-propagation).
...
<Frame caption="The Sources tab. Library sources and your own, each with its status and when it last fetched.">
  <img src="/images/user-guides/radar-signals-sources.png" alt="Radar sources list with Name, Type, Date Added, Last Fetched and Status columns" />
</Frame>

The table shows **Name**, **Type** (**Article feed**, **RSS feed**, **API**, **Snapshot**, **Black Kite** and so on), **Date Added**, **Last Fetched** (**Never** until the first successful run) and **Status**. Search by name or filter the list. The row menu has **Archive**, which stops collection and cannot be undone. Archived sources keep the items they already collected.

**Add source** offers three routes:

- **From Source Library**: tick sources in the curated catalog, narrowed by segment or search (**Add all visible** ticks everything in view), then **Connect**. A source Coverbase has withdrawn is greyed out as **Unavailable**, with the reason. The full list is on the [source library page](/products/source-library). Library sources are shared and run on a schedule Coverbase sets; you can rename one but not change how it fetches.
...
| **Paused** | Collection has stopped for this feed, usually because Coverbase withdrew a library source whose page no longer exists or refuses automated requests. Items already collected stay. |
...
**Add detector** (or **New detector** on the Radar page) offers **From detector library** and **Create custom detector**.

<Frame caption="The detector library. Templates by segment; the preview shows a template's guidance and the sources it brings with it.">
  <img src="/images/user-guides/radar-signals-detector-library.png" alt="Detector library dialog with segment chips, a list of templates and a preview showing guidance and sources" />
</Frame>

The library filters templates by **All**, **Recommended** and segment (**Cybersecurity**, **Financial**, **Operational**, **Legal**, **Compliance**, **Reputational**, **Contractual**, **ESG**, **Corporate**, **Geopolitical & External**, **AI Risk**). Click a template to preview its **Guidance**, the **Sources** it deploys with, and an **Example**. **Add detector** adds it with its recommended settings; **Configure** adds it and opens it for editing. On a template you already have, the button reads **Enable detector** or **Disable detector**. Tick several and **Enable** them at once. Every template is listed with its default severity on the [detector library page](/products/detector-library).

Enabling a template adds its sources to your organization, where you do not already have them, and scopes the new detector to them, so it starts working without further setup. A source marked **Unavailable** has been withdrawn and is skipped. A template becomes an ordinary detector on your organization, so everything below applies to it, including changing its sources.
...
| **Sources** | Restrict the detector to specific sources. Empty means every source. A detector added from the library starts limited to the sources it deployed with, so a source you add later is not read by it until you add it here. A detector limited to sources sees events you record by hand only if it includes **Manually-created events**. |
...
| **Positive examples**, **Negative examples** | Events the detector should and should not alert on. Dismissals and thumbs ratings land here. Remove an example to make the detector reconsider. |
...
<Tip>
  Start narrow. Keep the threshold at **Medium** or **High**, target the vendors you would act on, and watch two weeks of alerts before loosening anything. Every dismissal makes the detector quieter, so the first weeks are also when it learns fastest.
</Tip>
...
Vendors also carry a monitoring flag, which a workflow's **Enable vendor threat monitoring** action turns on. That flag is what the **vendors monitored by Radar** count on your subscription uses, so it decides what monitoring costs, not what a detector watches.

## Notifications

Radar sends four notifications. Each person can switch them on or off in their notification settings: **Radar detector alert** and **Reassessment trigger fired** sit in the monitoring group, **Case created** with cases, and **Dismissal mentions** with mentions.
...
- **Dashboards.** The **Portfolio risk** chart on the Radar page plots signals over time by severity, with a **3M**, **6M**, **12M** or **All** range and vendor and tag filters; click a point for the affected vendors.
...
| A detector from the library ignores a source you added later. | It is scoped to the sources it deployed with. Add the new source under the detector's **Sources**. |
| A dismissal did not stop similar alerts. | The person who dismissed it cannot edit detectors, so nothing was taught. Ask an admin to add the event as a negative example on the detector. |
