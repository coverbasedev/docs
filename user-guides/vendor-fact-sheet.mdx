...
description: "How to read the Financial Health Score and Security Posture cards on a vendor fact sheet, what each number is actually claiming, and what to do about what you find."
icon: "gauge-high"
hidden: true
---

<Info>
  This guide is part of the [User Guides](/user-guides/overview) collection. It covers the two scored cards on a vendor's fact sheet. For the product-level explanation of where the data comes from, see [Vendor Intelligence](/products/vendor-intelligence) and [Security Intelligence](/products/security-intelligence).
</Info>
...
Two cards on the fact sheet carry a score rather than a description: **Coverbase Financial Health Score** and **Security Posture**. Both are assembled without contacting the vendor — one from regulatory filings and registries, the other from the vendor's own public infrastructure.
...
<CardGroup cols={2}>
  <Card title="Financial health: the confidence tier" icon="layer-group">
    **A** means filed financial statements. **B** means estimates. **C** means no financials at all — the score was inferred from funding, headcount, and corporate standing. The tier is derived from what actually resolved, never declared.
  </Card>
...
<AccordionGroup>
  ...
  <Accordion title="The security score moved and nothing changed at the vendor." icon="arrows-up-down">
    Two things can cause this. A factor that was previously unmeasurable may now be measurable (or the reverse), which changes what the weighted mean is averaging over. And when your organization first enables the feature, the rating switches from seven factors to eleven with new weights — expect a one-time shift at that point.
  </Accordion>

  <Accordion title="Can I dispute a finding?" icon="flag">
    Every finding names the record it read, so start by checking it yourself — a DNS record or a response header takes a moment to verify. If the record has changed since the last refresh, trigger a fact-sheet refresh and the finding will clear on its own. If you believe the reading itself is wrong, raise it with your Coverbase representative with the vendor domain and the finding.
  </Accordion>

  <Accordion title="How often do these refresh?" icon="rotate">
    Both refresh on the regular fact-sheet cadence, and on any manual refresh you trigger from the vendor. Score history is kept as discrete points — one per day at most — so the trend line reflects separate computations rather than a smoothed curve.
  </Accordion>
</AccordionGroup>
...
- **The API** returns both on the vendor fact sheet endpoint. See the [API reference](/api-reference/vendors#vendor-intelligence-sections).
