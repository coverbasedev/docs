...
<Note>
  **A step that resolves to nobody is skipped, but a policy that resolves to nobody at all is refused.** If a policy asks for the vendor owner and the vendor has none, that step is skipped and the rest of the chain runs. If every step of the policy skips, the submission is refused and names why, because approving on the spot would waive the chain you configured. Give the contract an owner, or name approvers the policy can reach, then submit again.

  A contract submitted when your org has configured no policy at all is approved immediately. An org that has configured no chain is not asking for one.
</Note>
