...
A new contract goes through four stages: somebody raises a request, a person confirms the values read off the draft, the request walks an approval chain, and the approved agreement goes out for signature. The contract's **Workflow** tab lays them out as numbered steps in that order, and a contract cannot skip one.
...
<Steps>
  ...
  <Step title="Confirm the metadata">
    Coverbase reads the draft and fills in what it can. Open the contract's **Workflow** tab and confirm the values under **Metadata sign-off**. The request cannot proceed until somebody does.
  </Step>
  <Step title="Approve">
    Click **Submit for approval** on the **Approval chain** step. The request enters the chain your organization configured, and each approver is emailed in turn and records a decision.
  </Step>
  <Step title="Send for signature">
    Once the chain clears, the **Signature** step of the Workflow tab offers the send. The signed copy comes back and is filed on the same contract.
  </Step>
</Steps>

The button beside **Actions** in the contract header names the step the contract is waiting on (**Submit for approval**, **View approval**, **Send for signature**, or **Track signature**) and opens the Workflow tab. While a clause review has flagged clauses nobody has triaged, it reads **Continue clause review** instead.
...
| Field | What it is |
| --- | --- |
| **Contract Name** | What people will call this agreement. |
| **Services** | Optional. The services the contract covers. |
| **Status** | The lifecycle stage the contract starts in. |
| **Hold for my review** | Optional. Keeps the contract out of approval routing while you finish it. See below. |
| **Link Existing Documents** | Optional. Contract documents already on the vendor, such as the MSA an order form hangs off. |
| **Upload New Documents** | Optional. The pre-execution draft. |

The form does not ask for the type, value, or dates. Coverbase reads those off the attached draft, and you check or fill them in afterwards on the **Terms** card of the contract's **Overview** tab. You can attach more documents later from the contract's **Documents** tab.

<Tip>
  **Link the governing paper now, not later.** If this is an order form or an amendment, link the MSA it hangs off under **Link Existing Documents**. An order form with no MSA behind it reads as a standalone commitment in every report that follows.
</Tip>

### Starting a draft nobody is told about

Most requests are raised from a document you already have in front of you, and you can fill the record in one sitting. Some are not. When you are raising the record before the terms are settled, tick **Hold for my review** on the create form.

<Frame caption="A held contract. It cannot enter approval routing until someone releases it.">
  <img src="/images/user-guides/contract-intake-hold.png" alt="Needs Attention card with a Held for intake review line and a Release for routing button" />
</Frame>
...
The **Needs Attention** card on the contract's **Overview** tab says it is held, and so does the Workflow tab. Click **Release for routing** in **Needs Attention** when the record is ready to be looked at. Approval routing is then available in the normal way, and the release is recorded in the contract's activity trail with who did it.
...
## Asking a colleague to look first

The first step on the **Workflow** tab is **Reviews**. Click **Request review** to ask a named colleague or a user group for a decision on the draft, with a note saying what to look at. Each review records its outcome, and the step counts how many have been approved.

Reviews are optional. They do not hold up submission or the approval chain on their own, so a request can be submitted with a review still open. Use them when somebody should read the draft before approvers are asked, and use the approval policy for the decisions that must be recorded before the contract can move. Reviews work the same way everywhere in Coverbase; see [Reviews, approvals and gates](/user-guides/reviews-and-approvals).
...
Open the contract's **Workflow** tab. The **Metadata sign-off** step shows:
...
Fix anything that is wrong in **Terms** on the Overview, then click **Confirm metadata**. Coverbase records who confirmed it and when.
...
Each field records whether it was entered by a person or suggested by the AI. The **Metadata sign-off** step marks the source beside each value, and the same marker appears on the **Overview** tab. A reviewer can see which numbers a person typed and which came off the paper.
...
<Note>
  ...
  **A step that resolves to nobody refuses the whole submission.** Coverbase does not skip the step and run the rest, because a chain missing a rung still reports itself as approved. Two ways a step reaches nobody:
  ...
  A contract submitted when your org has configured no policy at all is approved immediately, and its **Approval chain** step says so rather than showing a bare **Approved** badge. If that is not what you expect, configure a policy.
</Note>

### Submitting

On the **Workflow** tab, click **Submit for approval** in the **Approval chain** step. Until the metadata is confirmed and every required value has one, the button is disabled and the step says what is still needed: a value to add under **Terms** on the Overview, the sign-off itself, or an intake hold to release.
...
An approver opens the contract's **Workflow** tab and sees **Approve** and **Reject** on the chain step waiting on them. The buttons appear only for the person being asked, so an approver who is not on this step sees the chain but no controls.
...
Once the chain clears, the **Signature** step of the Workflow tab offers the send. Before then the button is disabled and the step says why.

Which send you see depends on how your organization signs. Organizations on Coverbase's own e-signature see a **Signature** card that raises the envelope inside Coverbase; see the [signature overview](/user-guides/signature-overview). Organizations that sign in DocuSign see **DocuSign Execution** with **Send to DocuSign**, described below.
...
Coverbase needs to know who signs before the approval finishes. While the contract is still moving through its chain, **Set up automatic send** on the DocuSign Execution card asks the same questions the Send dialog asks: which document, who signs, in what order, and who is copied.
...
Once it is set up, the DocuSign Execution card says what is about to happen.

<Frame caption="The contract is still in approval, and this is what will go out when it clears.">
  <img src="/images/user-guides/contract-auto-send-planned.png" alt="DocuSign Execution card noting the contract goes out to two recipients automatically once approved" />
</Frame>
...
The **DocuSign Execution** card shows every DocuSign envelope raised on this contract, each recipient's state, and when the status was last read from DocuSign.
...
Everything is recorded on the contract's **Activity** tab: the submission, each review and decision, the comments, the status changes, the send, and the filing of the executed copy.

## The vendor's contract overview

The vendor's **Contracts** tab shows every contract you hold with that vendor in the same table as the contracts list, with the same columns and **Preview**.
...
**Actions → Export as Excel** exports the vendor's contract list. The export respects the filters and columns you are looking at.
