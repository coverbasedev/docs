...
1. Click **+ New Control Set → Upload Spreadsheet**, and download the template and the tips document that comes with it. You can also grab the same template below — its **Instructions** tab carries the tips.
...
<Card title="Download the control set template" icon="file-excel" href="/assets/coverbase-control-set-template.xlsx">
  The spreadsheet you fill in and upload to create a control set. The **Controls** tab is the one that gets imported; the **Instructions**, **Document Types**, and **Risk Domains** tabs are reference and tips.
</Card>
...
- Evidence sources: choose what the AI may draw on per control, from specific document types to web search to the vendor fact sheet. These combine and can be prioritized, so you might put the information security policy first and web search as a fallback.
