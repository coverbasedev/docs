...
If you assess at the service level, decide what that means for the vendor record. By default a service-scoped assessment scores the services it covers and leaves the vendor's own score alone, so a vendor whose payments service came back high can still read medium on its profile.

**Risk Roll-up** changes that. Find it under **Configuration → Scoring** on the **Risk Scoring** tab, below the risk scale itself. Two switches, one per risk type:

- **Roll up inherent risk from services**
- **Roll up residual risk from services**

Turn one on and every vendor that has not been given its own answer takes the **highest** score among its services for that risk type. Highest, not average, so one high-risk service is enough to make the vendor read high.

Services whose status falls in the inactive, discontinued, or denied group are left out. Retiring the service that was driving the score moves the vendor down to the next live one, without anyone editing the vendor by hand.

The two risk types are independent. Rolling up residual risk while leaving inherent risk manual is a reasonable setup: inherent risk comes from intake, residual risk comes from assessments.
...
<Warning>
  **Turning a switch on does not touch the scores you already have.** It applies from that point on, as assessments complete and services change. To apply it to your existing portfolio, use **Recalculate now** under **Recalculate existing vendors**. That rewrites the score of every vendor that has not been given its own answer, it cannot be undone, and it asks you to confirm first. Vendors carrying their own answer are left alone either way.
</Warning>

Recalculation runs in the background rather than while you wait, so a large portfolio takes a few minutes to settle. If neither switch is on, the button tells you there is nothing to recalculate instead of walking the portfolio to change nothing.

Individual vendors can depart from whatever you set here, and hand the decision back later. [Where a vendor's score comes from](/user-guides/running-an-assessment#where-a-vendors-score-comes-from) covers the per-vendor control.
...
2. Create detectors around what you actually act on, such as breaches and vulnerabilities, supply-chain disruption, or SEC and regulatory actions. Each detector takes plain-language guidance, example events to hone in on, impacted risk domains, and a severity threshold. Switch on its risk impact, and set the point range, if acting on its signals should move residual risk (see [risk propagation](/products/supplier-radar#risk-propagation)).
3. Test each detector before you enable it, and start with tight severity thresholds.

<Frame caption="Radar detectors, under Configuration → Radar. Each one is scoped to what you'll act on, with its categories and reviewers.">
...
<Warning>
  Tuning is what makes or breaks continuous monitoring. The failure mode is noise. One event makes one signal however many detectors fire, but a loose detector still fires on every routine patch note, and a single well-covered vendor can produce a dozen of those in a week. Keep detectors narrow and severities high at first, watch what fires for two weeks, then loosen deliberately. Fourth-party relationships get extracted automatically from SOC 2s, subprocessor lists, and SBOMs, so one event can implicate many vendors at once. That's exactly why precision matters.
</Warning>
