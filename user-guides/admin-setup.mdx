...
- Set the reassessment cadence in assessment settings. It defaults off residual risk (low might be every three years), but it can key off inherent risk instead, and it is fully overridable per vendor.
...
Radar is the external intelligence layer. Sources feed detectors, and detectors fire on the events that matter, which reach your analysts as signals. Sources and detectors live under **Configuration → Radar**. The **Configurations** button on the Radar page takes you there.
...
<Note>
  The full obligations workflow (validation, ownership, and stakeholder attestations) has its own dedicated guide, which we will add to this collection.
</Note>
