...
Open any evaluation and look at the evidence cards. Web-sourced evidence carries a colored badge next to the source:

- **Authoritative**: green
- **Reputable**: blue
- **Unreliable**: gray (you only see these if the source is set to **No minimum**)

Hover the badge to see the one-sentence reason the grader gave for that page. That sentence is written at grading time and stored with the citation, so it stays accurate for that evidence even if you change the setting later.
...
<AccordionGroup>
  <Accordion title="Does raising the bar make the AI stricter about compliance?" defaultOpen icon="circle-question">
    No. It makes the AI **more selective about what it reads**, not harsher about what it concludes. Evaluation strictness is a separate setting (lenient, standard, strict) on the control set. Credibility changes the inputs; strictness changes the judgment.
  </Accordion>
