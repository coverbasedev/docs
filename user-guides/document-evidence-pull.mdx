...
<AccordionGroup>
  ...
  <Accordion title="A question about a period" icon="calendar">
    ...
    The pull itself has no upload-date filter. The assistant uses upload dates when it lists the vendor's documents, and then points the pull at the document it found, or you can name that document yourself.
  </Accordion>
</AccordionGroup>
...
<Steps>
  <Step title="Filter on metadata">
    Your document type and service filters, or the one document you named, are applied in the database first. Nothing is read yet. At most 100 documents survive as candidates.
  </Step>
...
<Warning>
  The snippet **image** in the chat response is a short-lived link that expires about an hour after the result is collected. The Coverbase link does not expire. When you paste evidence into a ticket or a report, paste the Coverbase link, and save the image itself if you need it to persist.
</Warning>
