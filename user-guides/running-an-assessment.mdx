...
<AccordionGroup>
  <Accordion title="Inherent risk vs residual risk" icon="scale-balanced" defaultOpen>
    ...
    **Residual risk** is what remains once their controls, certifications, and documentation have been evaluated. The assessment is what moves you from one to the other.
    ...
    Both scores sit on the vendor, and both can also sit on individual services. [Where a vendor's score comes from](#where-a-vendors-score-comes-from) covers which one an assessment writes to.
  </Accordion>
...
<Steps>
  ...
  <Step title="Check the name, then create">
    Expand **Customize assessment name**. Names are generated from a template rather than typed by hand (the default produces something like "2026 Acme initial"), and a live example shows the result as you edit. Leave it alone unless you have a reason to change it; consistent names are what make three hundred assessments searchable a year from now.
...
An assessment writes its residual risk to whatever it was scoped to. Scope it to the vendor and the vendor's score moves. Scope it to two services and those two services get scored, and by default the vendor's own score is untouched.

That default surprises people. You complete a thorough assessment of a vendor's payments service, it comes back high, and the vendor profile still reads medium. Nothing has gone wrong. The score landed on the service.

**Risk Roll-up** connects the two. When an administrator turns it on for a risk type (see [Step 5 of the admin setup guide](/user-guides/admin-setup#decide-whether-service-scores-set-the-vendors-score)), a vendor takes the **highest** score among its services rather than carrying its own. Services that are inactive, discontinued, or denied drop out, so retiring the service that was driving the number moves the vendor to the next live one on its own.

Per-domain rows roll up the same way, not just the overall score. A vendor's Data Security row follows the highest Data Security score among its services.

<Note>
  Roll-up is off unless someone turns it on, and turning it on only affects scores from that point forward. If you are looking at a vendor whose scores predate the change, an administrator can apply it to existing records with **Recalculate now**.
</Note>

#### Overriding it for one vendor

A vendor can depart from the organization default. Open the score on the vendor's **Risk Profile** and use **Automatically infer from services**.

Switched on, the services own the score. The **Risk Score** field is read-only and **Clear risk score** is not offered, because anything you typed would be overwritten the next time the services changed.

<Frame caption="The vendor's residual score following its services. The field is read-only because the services set it.">
...
Switch it off and the field opens up for a score you set by hand, with the usual note field for recording why. That vendor now ignores the organization default in both directions: it will not follow a roll-up that is on, and it will not start following one that gets turned on later.

**Use organization default** appears once a vendor carries its own answer, and hands the decision back. Use it when a one-off override has outlived its reason.

<Frame caption="A vendor with its own answer. Use organization default returns it to whatever the organization is set to.">
