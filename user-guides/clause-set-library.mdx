...
<Warning>
  **A copied set needs its tiers rewritten before you rely on its verdicts.** The library's four tiers are four escalating *asks*, graded by how critical the vendor is: all four are positions you would be content to sign, and Critical is the one you would push for on a vendor holding regulated data. Clause review reads the same numbers as a scale of *acceptability* and maps them straight to verdicts, so tiers 1-2 score **Conforming**, tier 3 **Needs review**, and tier 4 **Non-conforming**. On a fresh copy that marks a vendor down for offering you the strongest terms in the set.

  Do not fix this by reversing the order, which only moves the problem: the Baseline draft is language the template says is acceptable from any vendor, and at tier 4 it would score Non-conforming. Instead pick the one draft that matches how critical this vendor is, make it your **tier 1**, and then write your own tier 3 (what you would sign after a round of negotiation) and tier 4 (what you would walk away from). The library gives you the ladder of asks; [the tier scale](/user-guides/contract-guardian#step-4-write-your-risk-tier-variants) needs a ladder of acceptability.
</Warning>
