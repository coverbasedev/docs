...
- The **NIST National Vulnerability Database (with KEV)** source connected. The recommended **Exploited Vulnerabilities & Supply Chain Compromise** detector adds it when enabled, so most organizations already have it. Otherwise add it under **Configuration → Radar → Sources → Add source → From Source Library**. See [Sources](/user-guides/radar-signals#sources).
...
2. Drag the file onto **Upload a CycloneDX or SPDX JSON bill of materials**, or click it to browse. One file at a time.
...
CycloneDX XML and SPDX tag-value files are not supported yet, and the upload box refuses them. If a vendor sends one, ask them for a JSON export, or convert it yourself with the CycloneDX or SPDX command-line tools.
