...
The most common mistake is uploading an SBOM in a format Coverbase stores but does not read, usually CycloneDX XML. The file is kept, but it has no components, so nothing can ever match it. Check the file's status after every upload.
...
| Format | Encoding | Read into components |
| --- | --- | --- |
| CycloneDX | JSON (`.json`, `.cdx`) | Yes |
| SPDX | JSON (`.json`) | Yes |
| CycloneDX | XML (`.xml`) | No. Stored as **Unsupported**. |
| SPDX | Tag-value (`.spdx`) | No. Stored as **Unsupported**. |

The upload box accepts `.xml` and `.spdx` files, so an upload in those encodings succeeds. It is only when parsing finishes that the status shows **Unsupported**. Convert the file to JSON (the CycloneDX and SPDX command-line tools both do this) and upload it again.
...
| --- | --- |
| Status **Unsupported** | The file is CycloneDX XML, SPDX tag-value, or not a bill of materials. Convert it to JSON and upload it again. |
