...
<CardGroup cols={2}>
  <Card title="Vendor Controls Library" icon="building-shield">
    **52 templates · 4,005 controls.** Frameworks, regulations and questionnaires you assess a *third party* against - ISO 27001, SIG, HECVAT, NIST CSF, HIPAA, PCI DSS and the rest. Opens from the **Vendor Controls** tab.
  </Card>
...
That is **73 templates** spanning **4,367 curated controls** in total. This page documents each one in more depth than the in-app picker: its full description, the risk types and domains it addresses, typical use cases, its control count, and, where the template is derived from a published framework, a link to the authoritative source.
...
Five templates for suppliers of physical goods and services, where the risk is a stopped production line rather than a breach. Unlike the frameworks above, these are written by Coverbase rather than derived from a single published standard, so they cite no external source. Individual controls do reference standards where one applies, such as ISO 9001 for quality systems or EU 10/2011 for food contact materials.

| Template | Controls | Focus |
| --- | ---: | --- |
| Supplier Resilience and Continuity <sup>NEW</sup> | 6 | Operational Risk, Third-Party Risk |
| Contract Manufacturing Oversight <sup>NEW</sup> | 6 | Operational Risk, Third-Party Risk, Audit Readiness |
| Logistics and Distribution Partner <sup>NEW</sup> | 6 | Operational Risk, Third-Party Risk |
| Raw Material and Chemical Supply Integrity <sup>NEW</sup> | 6 | Operational Risk, Regulatory Compliance, Third-Party Risk |
| Packaging and Product Compliance <sup>NEW</sup> | 6 | Regulatory Compliance, Operational Risk |

<AccordionGroup>
  <Accordion title="Supplier Resilience and Continuity · 6 controls">
    Continuity, alternate sourcing, lead time and audit readiness for suppliers whose failure stops a production line. Asks for a dated business continuity test rather than the existence of a plan, and treats an alternate source the supplier cannot name as no alternate at all.

    - **Risk types:** Operational Risk, Third-Party Risk
    - **Risk domains:** Business Continuity, Supply Chain
    - **Typical use cases:** Production-critical supplier review; Single-source exposure
    - **Coverage:** 6 controls
  </Accordion>
  <Accordion title="Contract Manufacturing Oversight · 6 controls">
    Quality systems, change control, batch traceability and undisclosed subcontracting for third parties manufacturing your product. The subcontracting control exists because onward subcontracting that nobody declared is the failure mode that surfaces during a recall.

    - **Risk types:** Operational Risk, Third-Party Risk, Audit Readiness
    - **Risk domains:** Manufacturing Quality, Supply Chain
    - **Typical use cases:** Contract manufacturer onboarding; Toll processing oversight
    - **Coverage:** 6 controls
  </Accordion>
  <Accordion title="Logistics and Distribution Partner · 6 controls">
    Chain of custody, cargo coverage, condition monitoring and contingency routing for freight forwarders, carriers and third-party logistics providers. Asks for a named list of every warehouse that will hold your goods, because a regional summary is not something you can act on later.

    - **Risk types:** Operational Risk, Third-Party Risk
    - **Risk domains:** Logistics, Business Continuity
    - **Typical use cases:** 3PL onboarding; Freight forwarder review
    - **Coverage:** 6 controls
  </Accordion>
  <Accordion title="Raw Material and Chemical Supply Integrity · 6 controls">
    Specification, composition disclosure, restricted substances, country of origin and responsible sourcing for resin, chemical and commodity inputs. Covers the disclosure depth your own regulatory obligations depend on, including substances below reportable thresholds.

    - **Risk types:** Operational Risk, Regulatory Compliance, Third-Party Risk
    - **Risk domains:** Supply Chain, Regulatory Compliance
    - **Typical use cases:** Raw material supplier qualification; Restricted substance diligence
    - **Coverage:** 6 controls
  </Accordion>
  <Accordion title="Packaging and Product Compliance · 6 controls">
    Food and product contact compliance, recycled content substantiation, packaging waste reporting and recall support for converters and packaging suppliers. Treats an unverified self-declared recycled percentage as a gap wherever you make onward regulatory or marketing claims from it.

    - **Risk types:** Regulatory Compliance, Operational Risk
    - **Risk domains:** Regulatory Compliance, Manufacturing Quality
    - **Typical use cases:** Packaging supplier qualification; Product contact compliance
    - **Coverage:** 6 controls
  </Accordion>
</AccordionGroup>
...
<AccordionGroup>
  ...
  <Accordion title="Usage, Seats & Licensing · 12 controls">
    What the organisation is paying for against what it is using. Seat waste, unassigned licences, dormant accounts and shadow expansion are financial findings that happen to be readable from the same admin surfaces as the security ones - so a run that is already logged in should collect them.
...
<AccordionGroup>
  <Accordion title="CRM Application Controls · 22 controls">
    A CRM holds the customer relationship: contacts, pipeline, contract terms, support history and whatever a salesperson attached to a record. It is usually the widest-reach application an organisation runs and the one with the most integrations hanging off it. Written against the surfaces a Salesforce, HubSpot, Dynamics or Zoho admin console actually exposes.

    - **Risk types:** Information Security, Data Privacy, Operational Risk
    - **Risk domains:** Data Protection, Identity & Access Management
    - **Typical use cases:** Continuous monitoring of the organisation's primary CRM; Detecting customer data leaving through a CRM integration; Sharing-model verification after a CRM administrator change; Pipeline and contract data exposure review
    - **Coverage:** 22 controls across 4 sections
  </Accordion>
  <Accordion title="HRIS & Payroll Application Controls · 18 controls">
    An HRIS holds the most sensitive employee data an organisation has - compensation, bank details, national identifiers, performance and health-related absence - and it is the system that drives access everywhere else. These controls read the access model, the identifier fields, the payroll change trail and the downstream provisioning that depends on it.
