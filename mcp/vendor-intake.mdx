...
The result is the same intake session (`cbintksn_...`) the portal produces. It lands in the same **Intake** queue, carries the same questionnaire answers, and waits on the same reviewer.
...
| **Prequalification** | Financial, security, and sanctions screening. Each check is separately switchable. |
...
Reviewers who prefer the dashboard lose nothing: sessions filed through chat appear in the **Intake** queue exactly as portal submissions do.
...
<CardGroup cols={2}>
  ...
  <Card title="Autonomous Intake" icon="inbox-in" href="/products/autonomous-intake">
    The product: classification, assessment plan selection, and downstream workflow orchestration.
  </Card>
