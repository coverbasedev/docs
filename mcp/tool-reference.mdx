...
Accepted parameters: `kind`, `vendor_id`, `document_id`, `query`, `llm_query_terms`, `run_id`, `document_type`, `document_status`, `service_id`, `uploaded_after`, `uploaded_before`, `limit`.
...
| `documents` | Evidence files for a vendor, or org-wide. Narrows on `query`, `document_type`, `document_status`, `service_id`, `uploaded_after`, `uploaded_before`. |
| `document_summary` | A summary of one document. |
| `document_download` | A download link for one document. |
| `document_evidence` | Starts reading the documents themselves to answer a question. Returns a run id, not the answer: a model picks which one or two documents to read (`limit`, default 2, max 5) and reads them. Takes a couple of minutes. |
| `document_evidence_result` | The answer from a started pull, by `run_id`: per document, an answer, a verbatim quote with its page, a confidence, a snippet image, and a link that opens the document highlighted. Omit `run_id` to list recent pulls. |
...
| `leadership` | The vendor's [People Intelligence](/products/people-intelligence) dossier: executives, board and key people, who reports to whom, registry filings that name them, and adverse-media and sanctions screening. Separates confirmed names from unconfirmed research candidates. |
| `security_posture` | The outside-in [security posture](/products/security-intelligence) measured against the vendor's own infrastructure: rating and grade, per-factor findings, actively exploited vulnerabilities, and lookalike domains. Requires Advanced company intelligence for your organization. |
...
Accepted parameters: `kind`, `assessment_id`, `eval_id`, `vendor_id`, `status`, `query`, `control_set_id`, `issues_only`, `unresolved_only`, `limit`, `full_text`, `analysis_offset`, `summary_offset`, `recommendation_offset`.

| `kind` | Returns |
| ------ | ------- |
| `search` | Assessments by vendor, status, or assignee. Each row carries the outcome and the residual risk, so a whole portfolio maps in one call per page. |
| `zero_touch` | One [Zero Touch](/products/zero-touch-assessments) run: its composite Zero Touch Score, the components behind it, whether the score was applied, and every source cited. |
| `zero_touch_leaderboard` | Vendors ranked by their latest Zero Touch Score, worst first, one row per vendor. |
| `zero_touch_audit` | Every review action on a Zero Touch run (approvals, escalations, corrections to its inputs) and what each did to the score. |
| `detail` | Full assessment: outcome, residual risk, control evaluations, findings with titles, people, documents, and the scope columns (date created, initiated by, run counter, products, services, control sets, parsed control counts). Carries the opening 500 characters of the AI summary as a preview. |
| `summary` | The complete AI-generated summary report: every section the summary prompt produced, plus the recommendation, the residual risk, and any notices. This is the **Summary** column of the Consolidated Assessment Report export. |
| `details_legacy` | The previous detail shape, for compatibility. Carries the outcome and residual risk too. |
...
<Card title="Replacing the Consolidated Assessment Report export" icon="table-columns" href="/mcp/assessment-exports">
  Every export column mapped to the call that returns it, with sample input and output.
</Card>
...
Reading a long summary report: `summary` works the same way. Pass `full_text=true`, or `summary_offset` (and `recommendation_offset` for the recommendation section), to page through a report longer than one response can carry. The truncation notice names the exact call that returns the next window.

### Risk scores and your scoring settings

Two settings under **Configuration → Scoring** change what every read returns, matching what the Consolidated Assessment Report export drops:

| Setting off | Effect |
| ----------- | ------ |
| Residual risk tracking | Residual risk is omitted everywhere, level and score alike. A workspace that turns this off does not compute the value, so the stored numbers are whatever the last computation left behind. |
| Numerical scores | Raw inherent and residual risk **numbers** are omitted and the named levels stay. The assessment score percent is a separate column and is unaffected. Vendors are still ranked by inherent risk in the portfolio report, without the score printed. |
...
Accepted parameters: `kind`, `vendor_id`, `engagement_id`, `contract_id`, `vendor_document_id`, `status`, `status_id`, `limit`.

| `kind` | Returns |
| ------ | ------- |
| `engagements` | Engagements, optionally scoped to a vendor. |
| `engagement_detail` | One engagement in full. |
| `contracts` | Contracts by vendor or status. `status_id` filters by one of your custom contract workflow statuses, from `query_platform(kind="statuses", target_type="contract")`. |
...
| `risk_register` | Risks on your [risk register](/user-guides/risk-register), highest residual first. Every status except closed unless you pass `status` (`proposed`, `open`, `mitigating`, `accepted` or `closed`). | `status`, `domain`, `vendor_id`, `top_n` |
| `risk_simulation` | A what-if scenario over your dependency chain from a plain-English `prompt` (an outage, breach, insolvency, sanction, exit, change, or offboarding). Nothing is saved; the loss estimate is withheld when its inputs are missing. | `prompt` |
...
<AccordionGroup>
  ...
  <Accordion title="eval / update triages one control evaluation" icon="check-double">
    Set `outcome` to `not_an_issue` to clear a false positive, `issue` to flag or reopen one, `mitigated` for a real gap the vendor fixed, or `risk_accepted` for one your organization chose to live with, and attach `review_notes`. Closing an issue does not change the score. This is the per-control correction, distinct from the whole-assessment sign-off in `review / create`.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Canned prompts" icon="wand-magic-sparkles" href="/mcp/prompts">
    Twenty scripted workflows you can invoke by name.
  </Card>
