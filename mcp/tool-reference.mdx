...
Accepted parameters: `kind`, `assessment_id`, `eval_id`, `vendor_id`, `status`, `query`, `control_set_id`, `issues_only`, `unresolved_only`, `limit`, `full_text`, `analysis_offset`, `summary_offset`.
...
Reading a long summary report: `summary` works the same way. Pass `full_text=true` or `summary_offset` to page through a report longer than one response can carry. The truncation notice names the exact call that returns the next window.
