...
| Identity | The signed-in user | Whoever the key was minted to act as |
| Access | Read/write, or narrower if the client asks | Read/write or read-only, fixed when the key is generated |
...
<Steps>
  <Step title="Generate a key">
    ...
    Two choices on that dialog decide what the key can do. **This key acts as** picks the identity behind it: yourself, a **service account** that needs no seat in your identity provider, or another user. **Read-only** withholds write permanently. If you are wiring up an automation that should only ever read, see [Programmatic read-only access](/mcp/permissions#programmatic-read-only-access).
...
- **A key carries the access of the identity it acts as.** A key that acts as you or another user carries that person's role, re-checked against their live membership on every request, so if their role changes the key follows it, and if they leave the organization the key stops working. For an unattended pipeline, have an admin generate the key as the **Service account**, which does not depend on any individual. A key that acts as anyone but you is always read-only.
