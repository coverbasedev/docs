...
The server exposes 12 tools, each dispatching many operations, across read and write capabilities.

<AccordionGroup>
  <Accordion title="Vendor intake, end to end in chat" icon="inbox-in">
  ...
  <Accordion title="RFPs" icon="clipboard-list">
    List RFPs and read one in full: timeline, rubric, invited vendors, submissions and their answers, and the stored cross-vendor comparison. Draft a brief, research candidate vendors, create and publish the RFP, compare submissions, and award it, each write with confirmation. See [How to run an RFP](/user-guides/running-an-rfp).
  </Accordion>
  ...
  <Accordion title="Tuning the AI and clearing false positives" icon="sliders">
...
<Card title="Full tool reference" icon="screwdriver-wrench" href="/mcp/tool-reference">
  Every route on all 12 tools: the read `kind` values, the report catalog and their parameters, and the complete write matrix.
</Card>
...
<CardGroup cols={2}>
  ...
  <Card title="Claude.ai" icon="comments" href="/mcp/connecting#claudeai">
...
<CardGroup cols={2}>
  ...
  <Card title="Tool reference" icon="screwdriver-wrench" href="/mcp/tool-reference">
    Every route on all 12 tools, with the parameters each accepts.
  </Card>

  <Card title="Canned prompts" icon="wand-magic-sparkles" href="/mcp/prompts">
    Twenty scripted workflows you can invoke by name instead of describing.
  </Card>
