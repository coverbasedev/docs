...
description: "Twenty scripted TPRM workflows you can invoke by name instead of describing what you want."
...
<AccordionGroup>
  ...
  <Accordion title="pull_document_evidence" icon="file-magnifying-glass">
    **Arguments:** `vendor_name`, `question`

    Answers a plain-English question out of a vendor's stored documents. Resolves the vendor, looks at what evidence exists, starts a pull, waits for it, then reports each answer with its verbatim quote, page number, confidence, snippet image, and a link that opens the document with the passage highlighted.

    Reading documents takes a couple of minutes, so the prompt starts the run and collects the result rather than blocking. See [Asking your documents a question](/user-guides/document-evidence-pull) for the full flow.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Tool reference" icon="screwdriver-wrench" href="/mcp/tool-reference">
    The 12 tools these prompts drive, and every route each one covers.
  </Card>
</CardGroup>
