...
<CardGroup cols={2}>
  <Card title="OAuth 2.0" icon="key">
    Access tokens expire after 24 hours and are renewed with a refresh token. Each refresh re-checks the user's organization membership and issues a new refresh token.
  </Card>
...
You can disconnect the connector at any time from the assistant client's connector settings, which removes the tokens that client holds. Coverbase has no separate switch to invalidate an OAuth access token that was already issued: it stays valid until it expires (up to 24 hours). Removing the user from your organization stops the next refresh, and an MCP API key revoked under **Configuration → MCP Server** stops working immediately.
...
Every tool call is logged with user identity, sanitized parameters, and outcome in Coverbase's security logs. Changes made through the connector run as the connected user and appear in the Coverbase audit trail attributed to that user, alongside actions taken in the dashboard. See [Audit trails](/security/audit-trails) for the full model: what is captured, where to review it, and how to export it.
