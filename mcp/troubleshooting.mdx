...
<AccordionGroup>
  <Accordion title="The connector authorizes but no tools appear" defaultOpen icon="circle-question">
    Confirm your role can connect. Admins, Members, Siloed Members and Guests can; Auxiliary Members and custom roles cannot. See [Roles and permissions](/mcp/permissions#role-to-access-matrix). If only the write tool is missing, the credential is read-only, which is expected for a Guest, a Siloed Member, or a read-only key.
  ...
  <Accordion title="The server asks for an OAuth token, but I'm using an API key" icon="key">
    The key is almost certainly not reaching the server. A request with no `Authorization` header is answered with `401` and a `WWW-Authenticate` challenge, which every MCP client reads as "start the OAuth flow", so a missing header looks identical to having no key at all.

    Check two things:

    - Your client config includes `"headers": { "Authorization": "Bearer cb_mcp_..." }`, or `--header` on the command line. A URL on its own is not enough. See [Choosing an authentication method](/mcp/connecting#choosing-an-authentication-method).
    - The key starts with `cb_mcp_`. An `ak_` key from **Configuration → API Keys** belongs to the [REST API](/api-reference/api-keys) and is not accepted here; MCP keys come from **Configuration → MCP Server**.
  </Accordion>
...
<AccordionGroup>
  <Accordion title="Rate limit errors" icon="gauge-high">
    The MCP server sits behind the same request-rate limits as the Coverbase API, counted for each combination of credential and IP address over a one-minute window. A client over the limit gets `403` responses until the window passes, so slow down and retry after a minute.
  ...
  <Accordion title="Tools are slow or timing out" icon="hourglass">
    Some routes (notably `get_report(report_name="org_activity")` and `query_vendors(kind="detail")`) consolidate many internal calls into one to save tokens, which can be slower on accounts with very large data volumes. Try narrower scoping (specific vendor ID, shorter time window).
  </Accordion>
</AccordionGroup>
