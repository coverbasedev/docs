...
## 2026-09-24: Security Intelligence tells you what to do next

The Security tab now opens with what to do and what changed, ranks a vendor among your other rated vendors, and carries both into reports and MCP.

### New features

- **Next steps.** Every security profile carries a ranked list of actions, most urgent first, each resting on a named observation: an actively exploited CVE, a database or remote-desktop port open to the internet, a failing certificate, DMARC that does not enforce, a mail-capable lookalike domain, staff credentials in a breach within the last year. **Ask the vendor** copies a question that quotes the observed value, and **Go to** jumps to the evidence. The rules run when the profile is read, so every vendor scanned before today has a list now. See the [security intelligence guide](/user-guides/security-intelligence#next-steps).
- **Since last scan.** Each refresh records what it found that the previous one did not: the score and factor movement, findings that appeared or were resolved, vulnerabilities that appeared or are no longer observed, assets that arrived or left. Each count opens to the names behind it, and the factor rows carry the same movement as a badge. A finding counts as resolved only when its factor was measured in both scans. See [since last scan](/user-guides/security-intelligence#since-last-scan).
- **Standing among your vendors.** With at least five rated vendors, the rating shows where this one sits among them, such as **Top 30%**, computed from the ratings as they stand when you open the page. See [standing among your vendors](/user-guides/security-intelligence#standing-among-your-vendors).
- **Jump links and a security timeline.** A row under the rating links to each section this scan produced, and the drawer at the bottom of the page lists every dated fact on the profile as one chronology: ratings, when each finding was first seen, breaches, assets arriving and leaving, and disclosed incidents.
- **In reports and exports.** The PDF export opens with the next steps and the change log. Report templates gain `{{intelligence::security next steps}}` and `{{intelligence::security change since last scan}}`, and the security posture section an agent or MCP client reads now carries both blocks.

### Documentation

- The [security intelligence guide](/user-guides/security-intelligence) gains sections on next steps, since last scan, standing among your vendors and the security timeline, with four troubleshooting rows. The [product page](/products/security-intelligence) explains how the rules are chosen and evaluated, and the [template placeholders](/reporting/template-placeholders#security-posture) reference lists the two new placeholders.

---

## 2026-09-10: Ask your documents a question

Ask a question in chat and Coverbase reads the vendor's documents to answer it, returning the passage the answer rests on.

### New features

- **Document evidence pulls.** Ask something like "does Acme's SOC 2 cover encryption at rest?" through a connected assistant. Coverbase filters the vendor's documents on metadata, lets a model pick the one or two most likely to hold the answer, reads them, and returns a direct answer with a verbatim quote, its page number, a confidence, and a cropped image of the page with the passage highlighted. See [Asking your documents a question](/user-guides/document-evidence-pull).
- **Links that open the document highlighted.** Every answer carries a link that opens the document preview on the right page with the passage marked. The highlight travels as the quote itself rather than as page coordinates, so it still resolves after a re-render that would move a stored bounding box.
- **Repeat questions reuse what was already quoted.** Passages earlier work cited from the same documents are offered to the selection step, so a question already answered can come back without opening anything. A passage whose underlying insight has since been re-extracted is never reused.
- **Scanned documents are readable.** A pull reads the searchable rendition produced at ingestion, so an OCR'd scan answers and highlights like a native PDF. Excel workbooks are read as workbooks and cite a sheet name instead of a page.
- **`pull_document_evidence` prompt.** A scripted version of the whole flow, in the prompt menu of any connected client. See [Canned prompts](/mcp/prompts).
- **Sharper document filters over MCP.** Listing a vendor's documents now narrows on free text, document type, status, service, and upload date range.

### Documentation

- New guide: [Asking your documents a question](/user-guides/document-evidence-pull), covering what comes back, how documents are chosen, reading an answer critically, and the limits.
- The [tool reference](/mcp/tool-reference) and [example prompts](/mcp/example-prompts) gain the evidence routes.

---

## 2026-09-09: Reputational risk on a Zero Touch run

A Zero Touch run now reports what it found against the vendor's name as its own section, in the app and in both exports.

### New features

- **Reputational risk section.** Adverse media on the vendor's leadership, open watchlist matches where your organization screens, and the run's own reputation controls now arrive as one section rather than three places to go looking. Findings are filed under five dimensions: media and customer sentiment, litigation exposure, regulatory and enforcement, executive conduct and stability, and sanctions and association. See the [Zero Touch guide](/user-guides/zero-touch-assessments#reading-the-reputational-section).
- **Not screened is never reported as clear.** Each dimension reports its own standing, and a dimension nothing searched reads **Not screened** rather than **Clear**. The section names every unscreened dimension, so a short findings list is not mistaken for a clean vendor.
- **Material findings first.** A matter the subject was cleared of, or one a reviewer has dispositioned, is still listed because it is part of the record of what the run read. It is not counted as material and it does not deduct from the score.
- **Second in both exports.** The section prints straight after the vendor identity in the PDF, with the overall standing on the first page. The spreadsheet carries the read on the summary sheet under the score and a **Reputational risk** tab beside it, one row per finding with its publisher, date and link. The portfolio export gains reputational standing and material finding count columns.

### API

- `GET /v1/zero_touch_assessment/{id}` returns `result.reputation`, carrying the overall standing, the five `dimensions` and the `findings` behind them. It is `null` on a run scored before this shipped, which means the run never assembled one rather than that the vendor is clear. See the [API reference](/api-reference/zero-touch-assessments#read-a-run).

### Documentation

- The [Zero Touch guide](/user-guides/zero-touch-assessments) gains a section on reading the reputational read, an updated export walkthrough, and a troubleshooting row on the difference between **Not screened** and **Clear**.

---

## 2026-09-05: Review a vendor's attributed estate

Security Intelligence now shows every domain and address it attributes to a vendor as a list you can correct, and every finding carries the step that resolves it.

### New features

- **Attributed estate.** The Security tab lists every asset the scan attributes to the vendor, with when each was first and last seen. Filter to what still needs a decision, and **Confirm** or **Dispute** each one from its row. A disputed asset leaves the next scan and the rating; withdrawing the dispute puts it back. See the [security intelligence guide](/user-guides/security-intelligence#attributed-estate).
- **Add the assets the scan cannot find.** Discovery reads certificate transparency, so a host with no public certificate never appears. **Add asset** puts them in the estate, one per line or comma-separated, and the next scan probes and scores them like any other. One submission costs one rescan however many hosts it carries, and assets you add survive a scan that cannot see them.
- **Remediation on every finding.** Opening a finding shows what was observed, whether it cost the rating points and how many, how long it has been open, and the step that fixes it. Forwarding one to a provider now hands them the change to make rather than the problem to research.
- **Generate findings in bulk.** **Generate findings** raises a tracked finding for every scored issue at once, ticked by default and ordered by severity, for the sweep that turns up the same problem on a dozen hosts.
- **Transport checks per host.** TLS is measured on each attributed host rather than only the vendor's main domain, so one neglected server does not hide behind a well-configured front door.

### Fixes

- DNSSEC, TLS reporting and email authentication no longer read as failures for domains that have simply not adopted them. Non-adoption is now recorded as an observation rather than scored like a misconfiguration, and DNSSEC is read from the signed zone rather than from the resolver's answer.

### Documentation

- The [security intelligence guide](/user-guides/security-intelligence) gains sections on opening a finding, raising findings from a scan, and reviewing the attributed estate, with new screenshots and three troubleshooting rows.

---

## 2026-09-01: Radar signals

Radar now works as one signal per event instead of the old per-detector alerts feed, with titles and summaries written for your organization.

### New features

- **Signals.** Every event your organization is exposed to appears once on the new **Signals** tab, as a signal carrying a status (Tracked, Open, In alert, Triaged, Dismissed), the highest severity any detector assigned, and the vendors and detectors involved. Each detector's verdict stays visible on the signal's **Detectors** tab. See the Supplier Radar [key concepts](/products/supplier-radar#key-concepts).
- **Tracked exposure.** A signal appears as soon as a vendor you depend on is linked to an event, before any detector fires, so fourth-party exposure is visible before anything has fired.
- **Generated titles and summaries.** Each signal's title and summary are written for your portfolio and regenerated when the detectors that fired change. Edit them by hand, click **Regenerate summary**, or set **Custom AI instructions** that apply to every summary in your organization.
- **Configuration moved.** Detectors and sources now live under **Configuration → Radar**, reached from the **Configurations** button on the Radar page.

### Documentation

- [Step 8 of the analyst guide](/user-guides/analyst-reviewer#step-8-work-radar-signals) and [step 9 of the admin guide](/user-guides/admin-setup#step-9-configure-radar-continuous-monitoring) are rewritten around signals, with new screenshots. The [field catalog](/fields/catalog#radar-signals) now mirrors the Signals tab, and the [Radar API](/api-reference/radar) reference explains how events and alerts map to signals.
