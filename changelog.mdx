...
Ask a question in chat and Coverbase reads the vendor's documents to answer it, coming back with the passage rather than a summary of it.
...
- **Links that open the document highlighted.** Every answer carries a link that opens the document preview on the right page with the passage marked. The highlight travels as the quote itself rather than as page coordinates, so it survives a re-render that would move a stored bounding box.
