...
- **Add an asset the scan cannot find.** Discovery reads certificate transparency, so a host with no public certificate never appears. **Add asset** puts one in the estate, and the next scan probes and scores it like any other. Assets you add survive a scan that cannot see them.
