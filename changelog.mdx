...
Radar now triages one signal per event instead of a feed of per-detector alerts, with titles and summaries written for your organization.

### New features

- **Signals.** Every event your organization is exposed to appears once on the new **Signals** tab, as a signal carrying a status (Tracked, Open, In alert, Triaged, Dismissed), the highest severity across its alerts, and the vendors and detectors involved. The alerts themselves stay visible on the signal's **Detectors** tab. See [how Radar organizes what it finds](/products/supplier-radar#how-radar-organizes-what-it-finds).
- **Tracked exposure.** A signal appears as soon as a vendor you depend on is linked to an event, before any detector fires, so fourth-party exposure is visible without an alert.
- **Generated titles and summaries.** Each signal's title and summary are written for your portfolio and regenerated when its alerts change. Edit them by hand, click **Regenerate summary**, or set **Custom AI instructions** that apply to every summary in your organization.
