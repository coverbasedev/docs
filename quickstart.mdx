...
By default a key carries no elevated permissions and can only call ordinary `/v1/*` routes. An admin can also mint a key with **admin scopes**: `keys:manage` (manage `ak_*` keys), `audit:read` (read the system audit log), `banking:read` (read full supplier bank and tax identifiers), and `review:sensitive_changes` (approve queued bank-detail changes). The admin-JWT path still works for the key and audit-log endpoints. See [API key scopes](/conventions#api-key-scopes) and the [API Keys reference](/api-reference/api-keys).
