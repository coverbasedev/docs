...
| Structured | A nested object. Filters can only test whether it is empty. |
...
| any of / none of | Text, Choice, Reference | Match against a list of values. |
...
| all of / not all of | Multi-reference, and choice or reference fields on related records | The record is related to every selected value. Extra values still match. |
| all are | Reference fields on related records | Every related record equals the selected value. |
...
<AccordionGroup>
  <Accordion title="Custom fields">
    Custom fields are defined per workspace, so they cannot be listed here.
    Custom fields can be attached to vendors, services, engagements, contracts,
    assessments, assessment responses, controls, findings, and reviews. Once
    configured, they appear alongside the built-in fields in the filter menus
    for vendors, services, contracts, assessments, assessment responses, and
    findings. Engagement and review custom fields are not filterable. See
    [Custom fields](/api-reference/custom-fields).
  </Accordion>
...
mixed AND/OR logic. A date value can be an ISO-8601 timestamp or a relative
duration such as `-P30D`.

<Note>
  The `/query` routes are the ones the dashboard uses. They accept a signed-in
  user's session token, not an `ak_...` API key, and return
  `{ "data": [...], "total_count": <int> }`. `limit` defaults to `50` and can be
  up to `500`. See [Conventions](/conventions) for authentication.
</Note>
