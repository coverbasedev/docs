...
<CardGroup cols={2}>
  ...
  <Card title="Assessment spreadsheet export" icon="table">
    One assessment's evaluations as `.xlsx` or `.csv`, from **Export as Excel** or **Export as CSV** on the assessment. Toggle the executive summary, follow-ups, notes, activity log, SLA timings and reviews. The `.xlsx` can also carry a sheet of the evidence behind each evaluation.
  </Card>
  ...
  <Card title="Due-diligence file" icon="box-archive" href="/reporting/due-diligence-file">
    The complete file for one third party: a full vendor report covering profile, assessments, findings, contracts, obligations, documents, monitoring, notes and the audit trail, plus the document archive behind it.
  </Card>
</CardGroup>
...
| One assessment's evaluations and evidence in a spreadsheet | Assessment spreadsheet export |
...
| Everything you hold on one third party, for an examiner, an auditor, or an acquirer | [Due-diligence file](/reporting/due-diligence-file) |
...
<Steps>
  ...
  <Step title="An analyst exports an assessment against that template">
    From the assessment page, choose **Export**, then **Export as Word**, and pick the template and a filename. The request returns immediately with a document ID; generation runs as a background job.
  </Step>
...
<Note>
  Custom Word report templates are gated per organization. If the export option is not visible on the assessment page, an admin needs to turn on **Assessment report templates** in **Configuration > Assessment Settings**. If it is visible but disabled, the assessment has no results to export yet (it is still collecting documents or being analyzed), or your role lacks permission to export assessments.
</Note>
