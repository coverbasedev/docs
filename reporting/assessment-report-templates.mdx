...
<Steps>
  ...
  <Step title="Author the .docx">
    Write your report in Word exactly as you want it to look. Put placeholders wherever data should appear. For a working example, click **Download Example** in the upload dialog. **Variables Catalog** in the same dialog lists every placeholder your organization can use, with its exact spelling.
  </Step>
  ...
  <Step title="Export an assessment against it">
    On any assessment, open the actions menu and choose **Export**, then **Export as Word**. Pick the template, confirm the file name, and click **Build Report**. Generation runs in the background: the dialog shows progress and offers **Download Report** when it is done, and you get a notification if you have notifications enabled. The export options stay disabled while the assessment is still collecting documents or being analyzed, and for anyone without permission to export assessments.
  </Step>
</Steps>
...
| `intelligence` | Vendor Intelligence on the vendor (company profile, financial health, security posture, registration, people and screening) and any financial statements submitted for it |
...
The agent is given vendor fields, assessment fields, evaluation summaries, findings, reviews, questionnaire excerpts, and the headline of each Vendor Intelligence section as preloaded context. It can call tools to fetch vendor data, assessment data, evaluation summaries, services, findings, reviews, commitments, questionnaire responses, and full Vendor Intelligence sections for anything not already loaded.
...
| `[Generation timed out]` | The prompt exceeded the per-prompt budget of 15 minutes |
...
- **The number ties a tab to a signer.** `/sn2/` creates the second signer, and `/title2/` or `/date_signed2/` place that signer's tabs. A tab anchor numbered for a signer with no matching `/snN/` in the document is ignored.
...
<AccordionGroup>
  <Accordion title="A known field that has no value" defaultOpen icon="circle-question">
    ...
    `{{intelligence::...}}` placeholders are the exception. An empty one renders `Not available`, because many of those fields are legitimately empty for a given vendor.
  </Accordion>
