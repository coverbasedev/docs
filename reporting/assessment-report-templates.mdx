...
Write the domain exactly as it is named in **Settings → Risk domains**; matching ignores case and surrounding spaces. A finding's domain comes from the control it was raised against — the control section's risk domain if it has one, otherwise the control set's. Findings not raised against a control (and controls with no domain at either level) appear in the unscoped tables only.
