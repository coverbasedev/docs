...
<Warning>
  Only `.docx` is accepted. `.doc`, `.dotx`, `.rtf`, and PDF uploads are rejected. If Word reports your file as `.docx` but the upload still fails, open it in Word and re-save as `.docx`; files renamed from another format fail the content check.
</Warning>
...
- **Formatting-tolerant.** A placeholder still resolves when Word has split it across several runs, and even when it spans a paragraph break.
...
The agent inherits the character formatting of the run the placeholder sits in, and the paragraph properties of the paragraph that hosts it. Put `{{agent::...}}` inside a bulleted paragraph and the generated list stays inside that list.
...
If the report is going out for signature, place DocuSign anchor strings in the template. Coverbase detects them during generation and records them on the generated document so a signature request can position tabs.
