...
From a vendor's page, open **Actions** and choose **Build report** to build a single branded PDF of the whole relationship. Choose the report type and the sections, and Coverbase builds it in the background and notifies you when it is ready. Building a report needs permission to export vendors across the organization, and the redesigned vendor and service reports are enabled per workspace: if **Build report** is missing, ask your Coverbase contact.
...
<AccordionGroup>
  ...
  <Accordion title="The vendor record as a spreadsheet" icon="table">
    **Export as Excel** in the same **Actions** menu builds a workbook of the vendor, one sheet per module of the vendor page: an overview, services, engagements, assessments, findings, contracts, obligations, documents, monitoring, performance and SLAs, procurement, notes and activity. Where the PDF caps each table so one record cannot dominate the read, the workbook carries every row, for filtering and sampling.
  </Accordion>
