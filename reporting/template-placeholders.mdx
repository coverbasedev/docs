...
The same list is in the product: **Variables Catalog** in the **Add new assessment report template** dialog shows every placeholder with its exact spelling, including your organization's enabled custom fields, and copies one when you click it.
...
| `{{reviews::outcome}}` | Review outcome label, or `Pending` while undecided |
| `{{reviews::description}}` | Review description |
| `{{reviews::notes}}` | Reviewer notes |
| `{{reviews::decided}}` | Date the review reached its outcome. An undecided review has no date |
...
| `{{intelligence::security next steps}}` | Recommended actions from the scan, most urgent first, joined with semicolons |
| `{{intelligence::security change since last scan}}` | What moved since the previous scan in one line, e.g. `since 2026-08-12: score -6, 2 new findings, 1 new CVE` |
...
| `{{intelligence::screening status}}` | Sanctions and adverse-media screening result for the vendor: `Not enrolled`, `Screened, no matches`, or the number of matches and how many await review |
...
| `title` | Finding title |
| `description` | Finding description, including recommendations |
| `status` | Finding status |
| `due date` | Due date, `YYYY-MM-DD` in the export timezone |
| `risk level` | Risk level from the risk matrix (urgency by severity) |
| `risk severity` | Criticality tier derived from the control's maximum score |
| `severity` | Severity set by the reviewer |
| `urgency` | Urgency set by the reviewer |
...
| `resolution timeline` | Days allowed to resolve a finding at this risk level, from your findings settings, as a number |
