...
Admins can list, create, rotate, and revoke `ak_*` keys self-service via the [API Keys API](/integrations/api-reference/api-keys) — no support ticket required. The API Keys endpoints themselves are JWT-only (a service account cannot manage other service accounts).
...
Bodies are never persisted. Records are visible to org admins via **Settings → Audit log** in the dashboard, filtered to the **Integrations** group, and via `GET /v1/system_audit_log`.
