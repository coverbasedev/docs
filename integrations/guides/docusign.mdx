...
- **From a contract.** The **DocuSign Execution** card on the contract's **Workflow** tab, once the contract has cleared its approval chain. This is the path described in [Contract intake and approval](/user-guides/contract-intake-and-approval).
...
- **Send on approval, without anybody pressing send.** A contract approval policy can carry **Send for signature on approval**. The requester names who signs while the contract is still in its chain, and the envelope goes out as them as soon as the last approver signs off. See [Sending automatically](/user-guides/contract-intake-and-approval#sending-automatically).
...
<Steps>
  <Step title="Open a send screen">
    Go to a contract's **Workflow** tab and click **Send to DocuSign** on the DocuSign Execution card, or open a vendor document's actions menu and choose **Send for Signature**. If you are not connected yet, Coverbase says so and offers **Connect DocuSign**.
  </Step>
  ...
  <Step title="Come back">
    DocuSign returns you to the screen you started from. The send dialog now shows your connected account instead of the connect prompt, and stays connected on your other devices too.
  </Step>
...
The send dialog's account card has a **Disconnect** action. It forgets your grant, so nothing can send as you until you authorize again. Envelopes already in flight are unaffected, and reconnecting takes one authorization round trip.

<Note>
  **Connect once, not once per device.** Your grant is held against your Coverbase user, so connecting on a laptop is also connected on a phone. Clearing site data does not disconnect you; **Disconnect** does.
</Note>

<Warning>
  **You can only ever send as yourself.** Coverbase resolves the DocuSign identity from your own grant, never from anything the screen sends, so no colleague's account is reachable from your session even if you can edit the contract. A person who has not connected sees the connect prompt rather than somebody else's account.

  An automatic send on approval is the one case where an envelope goes out with no browser open, and it is still your own grant: the envelope is sent as the person who **set up that send**, never as the submitter and never as whichever approver clicked last. Disconnecting stops those too.
</Warning>
...
6. If you want approvals to send on their own, an approval policy with **Send for signature on approval** under **Configuration → Organization → Contract Approvals**.
