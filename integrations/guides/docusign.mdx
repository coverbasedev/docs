...
<Warning>
  **You can only ever send as yourself.** Coverbase resolves the DocuSign identity from your own grant, never from anything the screen sends, so no colleague's account is reachable from your session even if you can edit the contract. A person who has not connected sees the connect prompt rather than somebody else's account.
  ...
  An automatic send on approval is the one case where an envelope goes out with no browser open, and it is still your own grant: the envelope is sent as the person who **submitted** the contract, never as whichever approver happened to click last. Disconnecting stops those too.
</Warning>
