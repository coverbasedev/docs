...
The connector requests an OAuth2 access token from your ProcessUnity tenant using a web-service name and password together with a ProcessUnity user's credentials, stored per-organization in Coverbase's secrets manager (never in configuration or code). An admin enters them on the **ProcessUnity** card under **Configuration → External Integrations** (**ProcessUnity base URL**, **Service name**, **Service password**, **ProcessUnity username**, **ProcessUnity password**), and **Test connection** confirms that the tenant issues a token. Sandbox and production tenants are configured independently, and Coverbase teams routinely stand the integration up against the sandbox tenant first.
...
1. ProcessUnity web-service credentials and an integration user for sandbox and production tenants.
