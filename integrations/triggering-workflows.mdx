...
The `trigger_workflow` parameter is optional. If specified, that named workflow is started alongside the assessment (a `404 workflow_not_found` is returned if no workflow with that name exists for the org). See the [Assessments API](/integrations/api-reference/assessments) for the full request and response schema.
...
See the [Workflows API](/integrations/api-reference/workflows) for the full schema.
