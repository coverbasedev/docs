...
<ParamField body="supporting_document_ids" type="string[]">
  Optional document IDs to attach as supporting evidence.
</ParamField>
...
The operation is a **soft delete**: the assessment row plus its evals and findings are marked `is_archived=true`, the assessment's portal (if any) is closed, and any in-flight workflow runs targeting the assessment are canceled in the workflow engine on a best-effort basis. The underlying rows are retained for audit purposes; archived assessments are excluded from list views and APIs by default.
...
<Note>
  Archive is idempotent at the row level: archiving an already-archived assessment returns `404` because archived rows are excluded from lookups by default. To restore an archived assessment, use the Coverbase dashboard.
</Note>
