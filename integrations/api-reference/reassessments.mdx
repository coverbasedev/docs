...
A **reassessment** (`cbrsm_...`) is a draft batch of assessments to run against a set of vendors — typically prompted by a [radar event or alert](/integrations/api-reference/radar). It lets you curate the vendor set and choose an assessment plan, then run it, without ever opening a case.
...
- [Radar API](/integrations/api-reference/radar) — the events and alerts that prompt reassessments.
