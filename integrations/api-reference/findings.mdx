...
Returns `201 Created`. **Exactly one of `vendor_id` or `assessment_id` is required** (at least one must be present).
...
<ParamField body="vendor_id" type="string">
  Vendor the finding is raised against (`cbvndr_...`). Required if `assessment_id` is omitted.
</ParamField>

<ParamField body="assessment_id" type="string">
  Assessment the finding is raised against (`cbqsrw_...`). Required if `vendor_id` is omitted.
</ParamField>
...
| Status | Body | When |
| ------ | ---- | ---- |
| 400 | `{"detail": {"code": "invalid_finding", "message": "Either vendor_id or assessment_id is required."}}` | Neither `vendor_id` nor `assessment_id` was provided. |
| 400 | `{"detail": {"code": "invalid_finding", "message": "..."}}` | Other business-rule rejection (e.g. bad reference ID). |
| 422 | Standard validation error | `title` missing or body malformed. |
