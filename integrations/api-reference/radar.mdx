...
- A **radar alert** (`cbrdrres_...`) is produced when a Radar detector flags a piece of source data as relevant to a vendor. Alerts are what you triage — dismiss the noise, or escalate the signal into a [reassessment](/integrations/api-reference/reassessments).
...
<ParamField body="primary_impact_type" type="string" required>
  Primary impact category. One of `infosec`, `operational`, `financial`, `legal_regulatory`, `reputational`, `privacy`, `esg`, `other`.
</ParamField>
...
<ResponseField name="severity" type="string">Severity (`info`, `low`, `medium`, `high`, `critical`).</ResponseField>
...
- [Reassessments API](/integrations/api-reference/reassessments) — escalate an alert/event into a batch of vendor reassessments.
