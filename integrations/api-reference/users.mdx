...
<ParamField body="email" type="string" required>
  The user's email. Used to link the account to your IdP on first SSO.
</ParamField>
...
| Status | Body | When |
| ------ | ---- | ---- |
| 409 | `{"detail": {"code": "user_exists", "message": "A user with email ... already exists."}}` | A user with that email already exists in this org. Expected on replay without an idempotency key. |
| 422 | Standard validation error | `email` missing, or `role` not a recognized value. |
