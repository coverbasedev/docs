...
Under **Configuration → Webhooks**, click a webhook's **Delivery history** button, or its **Health** value. The dialog lists each attempt with its **Event**, **Status**, **Response** (HTTP status and round-trip time) and **Attempted** time, newest first, with **View payload** to expand what was sent. The **Status** filter narrows the page you are looking at, and **Previous** and **Next** page through the rest.
...
<Note>
  `delivered` means a response was received, not that the receiver "succeeded". If your endpoint returns a non-`2xx`, the attempt is still recorded as `delivered` with that `response_status`, and it is not retried. Check `response_status`, not `status`, to find the events your receiver rejected.
</Note>
