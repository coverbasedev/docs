...
The public resource API is intentionally small and stable. It covers exactly these operations:

| Resource | Operations |
| -------- | ---------- |
| Vendor | Get, update (`GET`/`PATCH /v1/vendors/{id}`) |
| Assessment | Create, get (`POST`/`GET /v1/assessments`) |
| Workflow | Run, get run (`POST /v1/workflows/{name}/run`, `GET /v1/workflows/runs/{id}`) |
| Webhook | Create, list, update, delete, test (`/v1/webhooks`) |

Vendor and service creation, contracts, documents, controls, and findings are managed through the dashboard or the [Import API](/import-api) — they are not part of this resource API. Other domain objects (services, control sets, users, statuses) are referenced **by ID** in the requests and responses above, but are not independently CRUD-able through the public API.
