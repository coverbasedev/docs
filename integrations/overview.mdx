...
<CardGroup cols={3}>
  ...
  <Card title="Orchestration" icon="diagram-project">
    Once data lands, the workflow engine takes over. Triggers fire on object events, scheduled runs, or external invocation. Conditions branch the flow on risk score, tier, data class, jurisdiction, or contract value. Actions execute the work: create an assessment, send a portal to the vendor, raise review gates in parallel, and wait for their outcomes.
  </Card>
...
| Resource | Operations |
| -------- | ---------- |
| Vendor | Create, list, get, update, read and refresh the fact sheet (`POST`/`GET`/`PATCH /v1/vendors`) |
...
| Document | Upload, list, get, download, update, delete (`/v1/documents`) |
| Assessment | Create, get, archive (`POST /v1/assessments`, `GET`/`DELETE /v1/assessments/{id}`) |
| Finding | List, create, get, status-sync (`/v1/findings`) |
| Obligation | List, create, get, status-sync (`/v1/obligations`) |
| Radar | List and create events, list and triage alerts, list detectors and sources (`/v1/radar`) |
| Reassessment | Create, curate vendors, confirm, dismiss, run (`/v1/reassessments`) |
| Workflow | Run, schedule, list runs, get run with steps (`POST /v1/workflows/{name}/run`, `POST /v1/workflows/{name}/schedule`, `GET /v1/workflows/runs`) |
| Custom field | Discover definitions, read and write values (`/v1/custom-fields`) |
| Webhook | Create, list, update, delete, test, delivery history (`/v1/webhooks`) |
