...
| `pending_acknowledgement` | An internal control or a similar obligation matches the statement, but your organization requires a business unit to acknowledge it first. It becomes `satisfied` once that commitment is fulfilled. |
...
<ParamField query="obligation_status" type="string">
  Filter by status: `satisfied`, `not_satisfied`, or `pending_acknowledgement`.
</ParamField>
...
<ResponseField name="status" type="string">`satisfied`, `not_satisfied`, or `pending_acknowledgement`.</ResponseField>
...
<ResponseField name="assignee_group_id" type="string | null">Assigned user group (`cbdept_...`), if assigned to a group instead of a user.</ResponseField>
...
<ParamField body="status" type="string">
  `satisfied`, `not_satisfied`, or `pending_acknowledgement`. Defaults to `not_satisfied`.
</ParamField>
...
<ParamField body="assignee_group_id" type="string">
  Assign to a user group (`cbdept_...`) instead of a user. The group's assignment rule may pick a member, in which case the response carries that user in `assignee_id`.
</ParamField>
...
| Status | Body | When |
| ------ | ---- | ---- |
| 400 | `{"detail": {"code": "invalid_status", "message": "Invalid status '...'. Expected one of: satisfied, not_satisfied, pending_acknowledgement."}}` | `status` not one of the accepted values. |
...
<ParamField body="status" type="string">`satisfied`, `not_satisfied`, or `pending_acknowledgement`.</ParamField>
<ParamField body="assignee_id" type="string">Reassign to a user (`cbuser_...`).</ParamField>
<ParamField body="assignee_group_id" type="string">Reassign to a user group (`cbdept_...`).</ParamField>
...
| Status | Body | When |
| ------ | ---- | ---- |
| 400 | `{"detail": {"code": "invalid_status", "message": "Invalid status '...'. Expected one of: satisfied, not_satisfied, pending_acknowledgement."}}` | `status` not accepted. |
