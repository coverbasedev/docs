...
Several vendor endpoints accept/return a `primary_contact`. It is the address the inherent-risk-questionnaire portal invite (and other vendor emails) are sent to.
...
On read, `primary_contact` is derived from the vendor's stored contact personnel (first entry with an email), falling back to the vendor's security contact email; it is `null` if neither is set.
...
curl -X POST "https://sandbox.api.coverbase.app/v1/vendors" \
  ...
  -H "Idempotency-Key: eqx-vendor-test1" \
  -H "Content-Type: application/json" \
  -d '{
    "name": "EQX_Sup_Test1",
    "website": "https://supplier1.example.com",
    "use_case": "Demo supplier for Equinix sandbox",
...
resp = requests.post(
    ...
    headers={
        ...
        "Idempotency-Key": "eqx-vendor-test1",
...
curl -X POST "https://sandbox.api.coverbase.app/v1/vendors/cbvndr_e448ba62882143f3ba0c140bb2e30162/services" \
  -H "Authorization: Bearer ak_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "name": "EQX_Sup_Test1 — Primary Service" }'
```
...
```json
{
  ...
  "name": "EQX_Sup_Test1 — Primary Service",
  "description": null,
  "external_id": null,
  "tags": [],
  "created_at": 1746576000
...
<Note>
  `external_id` is set only at creation and is not updatable here.
</Note>
...
curl -X PATCH "https://sandbox.api.coverbase.app/v1/vendors/cbvndr_e448ba62882143f3ba0c140bb2e30162/services/cbsvc_3c2b1a098f7e6d5c4b3a2918374655ab" \
  -H "Authorization: Bearer ak_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "name": "EQX_Sup_Test1 — Renamed Service" }'
```
...
<ParamField body="risk_analyst_ids" type="string[]">Replace the risk analyst set (user IDs `cbuser_...`).</ParamField>
<ParamField body="relationship_owner_ids" type="string[]">Replace the relationship owner set (user IDs `cbuser_...`).</ParamField>
