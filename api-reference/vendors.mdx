...
<ResponseField name="custom_fields" type="object[]">Values your organization has set on this service. Empty array if none are set, and always empty on a create response. See the [Custom Fields API](/api-reference/custom-fields).</ResponseField>
...
<ParamField query="include_custom_fields" type="boolean" default="false">
  Populate `custom_fields` on each row. Off by default so listing stays cheap;
  single-vendor reads always include them. See the
  [Custom Fields API](/api-reference/custom-fields).
</ParamField>
...
<ResponseField name="items" type="object[]">
  ...
  `custom_fields` is empty unless you pass `include_custom_fields=true`.
</ResponseField>
...
<ResponseField name="custom_fields" type="object[]">Values your organization has set on this vendor. Empty array if none are set. See the [Custom Fields API](/api-reference/custom-fields) for the value shapes. Nested services carry their own `custom_fields`.</ResponseField>
...
<Note>
  `external_id`, `tier`, and `engagement_type` are **not** updatable through this endpoint (`external_id` is set only at creation; `tier` is derived by workflows). The request schema accepts `engagement_type`, but the value is ignored.
</Note>
...
{
  "vendor_id": "cbvndr_e448ba62882143f3ba0c140bb2e30162",
  "entity_id": "cbenty_a1b2c3d4e5f6a7b8c9d0e1f2a3b4c5d6",
  "status": "completed",
  ...
  "financials": {
    ...
    "financial_health": {
      ...
      "history": [{ "as_of": "2026-05-12", "score": 65, "band": "low", "confidence_tier": "C" }]
    }
  },
...
Three sections carry outside-in intelligence measured from registries of record and from the vendor's own infrastructure, rather than produced by a language model. All three cite their sources. See [Vendor Intelligence](/products/vendor-intelligence), [Financial Health Score](/products/financial-health-score), and [Security Intelligence](/products/security-intelligence) for what each field means.

<Note>
  The Financial Health Score and the measured security rating are optional features in beta, enabled per organization. Write your integration to treat `financials.financial_health` and `security_posture.external_intelligence` as optional. `financial_health` is `null` for an organization that does not have the score enabled. `external_intelligence` is present for every organization once generated, but without the measured rating it scores only the seven original factors. Any section can be `null` even when `status` is `"completed"`.
</Note>
...
<ResponseField name="financials.financial_health" type="object | null">
  The Coverbase Financial Health Score. `scorecard` holds `score` (0-100), `band` (the risk band, from `very_high` risk below 20 to `very_low` risk at 80 and above), `confidence_tier` (`A`, `B`, or `C`), `as_of`, the `pillars` breakdown (an object keyed by pillar: `financial_core`, `capital_position`, `momentum`, `adverse_events`, `corporate_standing`), `vitals`, `peer_group` and `peer_percentile`, `events`, `overrides_applied`, `provenance` (source → as-of date), and `unavailable_sources`. `history` carries prior scores for trend.

  Returned only when your organization has the Financial Health Score enabled; `null` otherwise.
</ResponseField>

<ResponseField name="financials.financial_health.scorecard.pillars.<pillar>.contributions" type="object[]">
  ...
  - `detail_key`: the category behind a non-numeric input (`unknown`, `dissolved`, `down`). **When this is present, `value` is not a measurement.** A `score` of 50 with `detail_key: "unknown"` means the input could not be determined, not that it was measured as mediocre, so render the two cases differently
...
<ResponseField name="security_posture.external_intelligence" type="object | null">
  ...
  Present for every organization once the security posture has been generated. Without the measured security rating, `factor_scores` covers only the seven original factors (see below).
</ResponseField>

<ResponseField name="security_posture.external_intelligence.recommended_actions" type="object[]">
  Ranked next steps derived from the profile when it is read, most urgent first. Each carries a `code` (for example `kev_exploited`, `dmarc_not_enforcing`), a `priority` of `urgent`, `soon` or `routine`, a `count`, the `subjects` it rests on (CVE ids, hosts, domains), and the `factor` it relates to. Empty when no rule fired.
</ResponseField>

<ResponseField name="security_posture.external_intelligence.changes" type="object | null">
  What this scan found that the previous one did not: `previous_as_of`, `previous_score`, `score_delta`, `factor_deltas`, `new_findings`, `resolved_findings`, `new_vulnerabilities`, `resolved_vulnerabilities`, `new_assets` and `departed_assets`. `null` on a first scan and on a profile stored before the comparison existed.
</ResponseField>
...
<ResponseField name="security_posture.external_intelligence.factor_scores[].findings" type="object[]">
  ...
  - `subject`: what the finding is about, such as a host, a DKIM selector, a cookie name, or a CVE
