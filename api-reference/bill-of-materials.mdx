...
All endpoints are org-scoped. They are the endpoints the dashboard's **Bill of Materials** tab uses, so they take a signed-in user's dashboard session token (a JWT): an `ak_` API key is rejected with `401 Unauthorized role.` See [Availability](#availability) below, and [API conventions](/conventions) for IDs, timestamps, and the error envelope.
...
| `POST` | `/v1/bill_of_materials/{bom_id}/promote` | No |
...
<ResponseField name="bom_metadata" type="object">Free-form metadata from the source document header (tools, authors, timestamp, lifecycle). Empty until parsed.</ResponseField>
...
<ResponseField name="components" type="object[] | null">The parsed [components](#the-component-object). Present on **detail** responses only (`/latest`, `/{bom_id}`, and the upload response); omitted from list summaries and from the promote and archive responses.</ResponseField>
...
Upload the file first with the presigned POST from `GET /v1/file/uploadable_url?extension=json`, then pass the uploaded file's URL here.
...
<ParamField body="s3_url" type="string" required>
  The URL of the uploaded file: the presigned `url` from `GET /v1/file/uploadable_url` followed by the `key` it returned in `fields`.
</ParamField>
...
curl -X POST "https://sandbox.api.coverbase.app/v1/bill_of_materials" \
  ...
  -d '{
    ...
    "s3_url": "https://coverbase-uploads.s3.amazonaws.com/7c1d9e2b41f04c5e8a3b6d0f9e2c4a71",
    "size": 20480,
...
Returns the full upload history for a vendor or service, **newest upload first**. The first item is the currently applied BOM unless an older upload has been promoted since. Items are summaries without `components`; use [Retrieve a BOM](#retrieve-a-bom) or [List components of a BOM](#list-components-of-a-bom) to pull components.
...
<ParamField query="limit" type="integer">
  Page size. Default `25`, range `1`-`500`.
</ParamField>
  ...
  "data": [
    {
      ...
      "created_at": 1748822400,
      "updated_at": 1748822460,
      "document": { "id": "cbvdoc_1a2b3c4d5e6f7a8b9c0d1e2f3a4b5c6d", "name": "acme-sbom.cdx.json", "s3_url": "https://coverbase-uploads.s3.amazonaws.com/7c1d9e2b41f04c5e8a3b6d0f9e2c4a71", "size": 20480, "extension": "json", "created_at": 1748822400 }
    }
  ],
  "total_count": 3
}
...
| Status | Body | When |
| ------ | ---- | ---- |
| 404 | `{"detail": "Bill of materials not found"}` | `bom_id` not found or not in your org. |
...
## Promote a historical BOM

<ParamField path="method" type="POST">
  `POST /v1/bill_of_materials/{bom_id}/promote`
</ParamField>

Makes an older, non-archived upload the applied BOM for its vendor and for every service it is linked to, until a newer upload or another promotion replaces it. The change is recorded in the vendor's (and each linked service's) activity log.
...
The response is the promoted [bill of materials object](#the-bill-of-materials-object), without `components`. An unknown or archived `bom_id` returns `404` with `{"detail": "Bill of materials not found"}`.
...
The response is the updated [bill of materials object](#the-bill-of-materials-object) with `is_archived: true`, without `components`.
...
Bill of Materials is available to every organization: the **Bill of Materials** tab appears on each vendor and service page, and no entitlement is required. Because these are dashboard endpoints, call them with a signed-in user's session token rather than an `ak_` API key.
