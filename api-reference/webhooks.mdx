...
<ParamField body="url" type="string">
  New endpoint. If provided, it must pass the same [URL restrictions](/integrations/webhooks#url-restrictions) as on create.
</ParamField>
...
curl -X PATCH "https://api.coverbase.app/v1/webhooks/cbwh_d4e5f6a7b8c9d0e1f2a3b4c5d6e7f8a9" \
  -H "Authorization: Bearer ak_live_xxx" \
  -H "Content-Type: application/json" \
  -d '{ "events": ["Vendor.Created", "Vendor.Updated", "Assessment.Updated"] }'
```
...
WH=$(curl -sS -X POST "$CB/v1/webhooks" "${A[@]}" -d '{
  "url": "https://hooks.example.com/coverbase",
  "events": ["Vendor.Created", "Assessment.Updated"],
...
# Changing the allowlist and whoami take an admin's dashboard session token, not an ak_ key:
J=(-H "Authorization: Bearer $ADMIN_JWT" -H "Content-Type: application/json")
# Configure (CIDRs validated server-side; invalid → 422):
curl -sS -X POST "$CB/v1/org/$ORG_ID" "${J[@]}" \
  -d '{"api_ip_allowlist":["203.0.113.0/24","198.51.100.7/32"]}'
curl -sS -X POST "$CB/v1/org/$ORG_ID" "${J[@]}" \
  -d '{"api_ip_allowlist":["not-a-cidr"]}'        # → 422
# Read back with the API key (org object now returns api_ip_allowlist):
...
curl -sS -X POST "$CB/v1/org/$ORG_ID" "${J[@]}" -d '{"api_ip_allowlist":[]}'
# Find the IP the API sees for the caller (same value the allowlist evaluates):
curl -sS "$CB/v1/whoami/ip" "${J[@]}"          # → {"client_ip":"203.0.113.42"}
```
