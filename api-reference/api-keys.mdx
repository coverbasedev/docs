...
| `banking:read` | Read full supplier bank account and tax identifiers instead of the masked last four characters. |
| `review:sensitive_changes` | Approve or reject a queued sensitive supplier change, and write bank details without waiting for review. |

The dashboard's **API keys** page (**Configuration**, under **Developers and integrations**) offers the same scopes under **Admin scopes (optional)** when an admin creates a key. See [API key scopes](/conventions#api-key-scopes) before granting the last two.
...
<ResponseField name="scopes" type="string[]">
  The admin scopes granted to the key. Each value is one of `"keys:manage"`, `"audit:read"`, `"banking:read"`, or `"review:sensitive_changes"`. A standard integration key returns an empty array (`[]`).
</ResponseField>
...
Atomically rotates a key: mints a new key, then revokes the original. The new key keeps the original name with a `(rotated YYYY-MM-DD)` suffix, because the identity provider rejects duplicate names; a second rotation on the same day adds a counter, such as `(rotated 2025-05-07 2)`. Use this when a secret may have been compromised, or as part of a scheduled rotation policy. **The new key inherits the same scopes as the rotated key**, so a scoped admin key stays scoped after rotation.
...
{
  "id": "ak_5ZmN...newkey",
  "name": "GRC pipeline (Acme) (rotated 2025-05-07)",
...
| 401 | `{"detail": "Unauthorized role."}` | A JWT whose role is not `admin`. An unknown or revoked key returns `{"detail": "Invalid or expired API key."}`. |
| 403 | `{"detail": "Not authenticated"}` | No `Authorization` header. |
