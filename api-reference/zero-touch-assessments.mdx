...
All endpoints are org-scoped. They are the endpoints the dashboard's Zero Touch pages use, so they take a signed-in user's dashboard session token (a JWT); an `ak_` API key is rejected with `401 Unauthorized role.` Most error bodies carry a free-form `error` key rather than a code. See [API conventions](/conventions) for IDs, timestamps, and the error envelope.
...
<ParamField body="instruction" type="string">
  A correction in plain language, up to 2000 characters, applied to the vendor before the run starts. Use it when the vendor record points at the wrong company or website: research reads the vendor's website at launch, so a correction made afterwards does not change what the run read.
</ParamField>
...
| `400` | The vendor ID is unknown or belongs to another organization, or `template_key` is not a shipped template. |
...
<ParamField body="vendor_ids" type="string[]" required>
  Vendor IDs to assess, from 1 to 200.
</ParamField>

<ParamField body="template_key" type="string">
  Switch your organization's Zero Touch scope to this template before launching, as on a single launch.
</ParamField>
...
Returns the single-page result: who the vendor is, what turned up against its name, the composite, the components behind it, control-set coverage, open issues with their evidence, and every source cited.
...
<ResponseField name="result.reputation" type="object | null">
  What the run found against the vendor's name, gathered from adverse media, open watchlist matches and the run's own `ZT-REP-*` controls. Carries an overall `standing`, a one-line `headline`, `finding_count` and `material_count`, a `dimensions` array, and a `findings` array. `null` on a run scored before this section existed, which means the run never assembled one rather than that the vendor is clear.
</ResponseField>

<ResponseField name="result.reputation.dimensions" type="object[]">
  One entry per dimension (`media_sentiment`, `litigation`, `regulatory`, `executive_conduct`, `association`), each with its `label`, its own `standing`, `finding_count`, `material_count` and a `detail` line.
</ResponseField>

<ResponseField name="result.reputation.findings" type="object[]">
  One entry per reputational matter, ordered material first then by severity. Each carries the `origin` that raised it (`adverse_media`, `screening`, `control`), the `dimension` it is filed under, a `headline` and `summary`, the `subject` it attaches to (`null` for the company itself), a `severity`, an `outcome`, `publisher`, `published_at`, `jurisdiction`, `source_urls`, `is_disputed` and `is_resolved`.
</ResponseField>

<Warning>
  **A dimension nothing searched reads `not_screened`, never `clear`.** The four standings are `adverse`, `watch`, `clear` and `not_screened`. `clear` means something searched and found nothing; `not_screened` means nothing searched, and carries no information about the vendor. `coverage_note` names every unscreened dimension. Do not treat an empty `findings` array as a clean record without reading `dimensions`.
</Warning>
...
The `POST` takes `{"template_key": "..."}` and points future runs at that template. It only ever re-designates control sets this module provisioned, so a set designated for you by Coverbase is never disturbed. An unknown key returns `400`; an organization that runs a control set of its own gets `409`.
...
<ParamField body="action" type="string" required>
  `approved`, `escalated`, `archived`, or `note`.
</ParamField>
...
<ParamField body="vendor_status_id" type="string">
  Optional. A vendor status (`cbst_...`) to set on the vendor along with the decision. Requires permission to update the vendor.
</ParamField>

Escalating records the judgement; it does not launch a full assessment. Archiving takes the run out of the working set and needs permission to archive assessments.
...
<ResponseField name="data[].action" type="string">
  `approved`, `escalated`, `corrected`, `rescored`, `revised`, `restored`, `rerun_requested`, `note`, or `archived`.
</ResponseField>
...
<ParamField body="auto_review" type="boolean">
  Whether a finished run is checked a second time against the vendor's own pages, with the supported changes applied as a new version of the result. On by default.
</ParamField>
...
There is no separate list endpoint, deliberately. A Zero Touch run is an assessment. Use the assessment query endpoint, `POST /v1/assessment/query`, with a rule on the `is_zero_touch` filter path (see the [field catalog](/fields/catalog#assessments)):
...
All three are read-only and org-scoped to the authenticated user. See the [MCP overview](/mcp/overview).
