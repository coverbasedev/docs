...
A Zero Touch Assessment is a lightweight triage run assembled entirely from open-source research, with no document request, no questionnaire and no vendor contact. It produces a single 0–100 **Zero Touch Score** so a portfolio can be ranked before real review effort is spent, and it does not draw down your assessment quota.
...
<Note>
  Organizations are provisioned the shipped templates on first use, so a `409` is unusual. It means every template has since been un-designated or archived — a deliberate act that provisioning will not undo.
</Note>
...
<ResponseField name="result.profile" type="object | null">
  The researched company profile: `company_name`, `url`, `description`, `industry`, `use_case`, `services`, `hq_location`, and `confidence` — how sure the run is it profiled the right company. `null` until the research completes, which happens shortly after launch and independently of the controls.
</ResponseField>
