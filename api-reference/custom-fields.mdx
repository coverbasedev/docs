...
<ParamField body="values" type="object[]" required>
  One to 100 `{ "config_id": "cbcfc_...", "value": ... }` entries. A `config_id` may appear only once per request.
</ParamField>
...
Every assignment in the request is checked before any of them is written, so a request naming one bad field changes nothing at all. Errors from this endpoint return `detail` as a plain string that names the field, for example `{"detail": "'Review Date' expects an ISO date (YYYY-MM-DD)."}`.

| Status | When |
| ------ | ---- |
| 400 | A value's type doesn't match the field (`number` into a `string` field, and so on), or a `date` value is not an ISO date (`YYYY-MM-DD`) |
| 400 | A `select` or `multiselect` value isn't one of the field's configured option labels |
| 400 | A `user` value names a user ID that doesn't exist in your organization |
