...
<Note>
  **Smart limits.** Uploads are capped at **100 MiB** per file. Allowed file
  ...
  `ppt`, `pptx`, `png`, `jpg`, `jpeg`, `gif`, `webp`, `zip`, `msg`, `eml`. Anything
  else is rejected with `400 unsupported_file_type`; an oversized file is rejected
  with `413 file_too_large`. List responses page at `limit` 1-200 (default 50).
</Note>
...
There are three ways to link a document to an assessment:
...
| `400` | `invalid_extension` | No file extension could be read from the file name. |
