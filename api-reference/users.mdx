...
Only these exact values are accepted; anything else, including shorthand such as `member` or `admin`, is rejected with `422`. On read, `role` can also be `org:custom` for a member who holds a custom role defined in your organization. That value cannot be assigned through this API.

If your organization uses custom roles and permissions, a caller can only assign a role whose permissions it holds itself. Assigning a role above the caller's own permissions returns `403 role_escalation`.
...
| Status | Body | When |
| ------ | ---- | ---- |
| 403 | `{"detail": {"code": "role_escalation", "message": "...", "offending": [...]}}` | `role` grants permissions the caller does not hold. |
...
Lists the org's users, newest first, or resolves a single user by email. By default the list holds invited and active members, the people the dashboard shows. This is the supported way to build an **email → user ID** mapping when syncing assignments (relationship owners, watchers, risk analysts) from an external system such as ServiceNow; those fields take Coverbase user IDs, so resolve the email here first.
...
<ParamField query="include_archived" type="boolean">
  Also include archived users and deprovisioned (`inactive`) memberships. Defaults to `false`.
</ParamField>
...
| 409 | `{"detail": {"code": "role_requires_invitation", "message": "Invite this person to the organization to give them a role."}}` | The person has a Coverbase account owned by another organization and is only named on your records. Invite them from the dashboard to give them a role. |
