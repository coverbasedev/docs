...
An **intake session** (`cbintksn_...`) is a vendor intake request. Creating one through this API registers the vendor, creates the intake session, and kicks off AI processing that auto-answers the selected inherent risk questionnaire(s). With `auto_submit`, the session is then submitted to the dashboard's **Intake** queue, where reviewers approve or deny it; on approval, the questionnaire-derived inherent risk scores are applied to the vendor and its services.
...
<Tip>
  **Not embedding intake in another system?** The same request can be filed conversationally through the MCP server, with the assistant researching the vendor, confirming its identity, running your configured screening, and walking the questionnaire with the requester. See [Vendor intake in chat](/mcp/vendor-intake). Both paths produce the same session and the same review queue.
</Tip>
...
`201 Created` with the new `session_id`. Unknown questionnaire IDs return
`404`, and so does an organization whose vendor intake is turned off. Replaying the same `external_id`
...
<ParamField body="context" type="string">
  ...
  Roughly the first 20,000 characters are included in each
...
<ParamField body="auto_submit" type="boolean" default="false">
  Submit the session for review once auto-answering completes. The request
  moves to **Pending review** in the **Intake** queue with its answers marked
  ready for review, and a person still approves or denies it. Leave it `false`
  to collect the answers over this API: the session stays a **Draft** and is
  not submitted for review. Every questionnaire must be one an intake reviewer
  can approve or deny; an `assessment` questionnaire returns `422`.
</ParamField>
...
| `complete` | Answers generated. With `auto_submit`, the request is submitted for reviewer triage in the dashboard |
...
<Note>
  Until it is submitted, an intake session created by this API is a **Draft**
  that no reviewer is asked to act on. With `auto_submit`, it is submitted as
  soon as its answers are generated and then follows the same triage and
  approval flow in the dashboard's **Intake** queue as a portal-submitted
  request.
</Note>
