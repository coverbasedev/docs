...
<ParamField body="assessment_plan_id" type="string">
  Assessment plan to run (`cbaspl_...`). Optional; set later before running.
</ParamField>
...
<ResponseField name="assessment_plan_id" type="string | null">Linked assessment plan (`cbaspl_...`).</ResponseField>
...
Returns an `items` array of [reassessment objects](#reassessment-object), newest first, with `total`, `limit`, and `offset`. Every match is returned in one page.
...
<ParamField body="assessment_plan_id" type="string">Assessment plan to run (`cbaspl_...`).</ParamField>
...
Create an assessment from the linked plan for each **included** vendor that does not have one yet, then mark the reassessment `run`. It can run from `draft` or `confirmed`. Requires `assessment_plan_id` to be set first; otherwise `400 missing_assessment_plan`. This **never** creates a case.

When the reassessment is linked to a radar event, running it also marks that event's signal as triaged and applies the risk impact of the linked alerts (or, if none were recorded, the event's active alerts) to the vendors.
...
| 422 | `{"detail": {"code": "reassessment_immutable", ...}}` | Editing or running a `run`/`dismissed` reassessment. |
| 422 | `{"detail": {"code": "invalid_reassessment", ...}}` | The request was rejected, for example a vendor or plan that does not resolve. |
