...
The dashboard rolls an event's alerts up into a **signal**: one row per event carrying your organization's status, title, and summary. Signals are not addressable through the API. Read the event for what happened, and the alerts for which detector flagged which vendor. See [how Radar organizes what it finds](/products/supplier-radar#how-radar-organizes-what-it-finds).
