...
Returns `201 Created`. Use this to seed an event into Radar without waiting for a source feed to pick up the underlying article; useful for recording out-of-band incidents and for exercising your detectors against a known event. Supplied vendors are linked to the event as **primary-impacted**. Set `create_alert` to run the detectors against it — see [detector evaluation](#detector-evaluation).
...
<ParamField body="create_alert" type="boolean">
  ...
  [radar alerts](#list-radar-alerts) for the ones that fire. Default `false`.
</ParamField>

### Detector evaluation

The scheduled Radar pipeline is source-driven: it evaluates what its configured
sources pull in, and never sees an event created through this endpoint. An event
posted here is therefore inert unless you set `create_alert`, which runs the
detectors out of band.
...
- **Vendors.** Only the vendors in `vendor_ids` that resolve within your org. An
  event with none is stored but never evaluated.
...
- **Cost.** One model call per (detector, vendor) pair, which is why this is opt-in.
